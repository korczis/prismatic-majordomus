...
/// The projection's own routes, resolved from the surfaces that declare them rather than
/// listed here: what answers, where, and whether a publication can carry it.
///
...
/// let swagger = routes.iter().find(|r| r.path == "/docs").unwrap();
/// assert_eq!(swagger.availability, majordomus_cli::web::Availability::ServedOnly);
