...
for href in $(grep -oE 'href="[^"]*/(features|profiles|guarantees|commands|why|doctrines)/[a-z0-9_-]+/"' "$P/index.html" | sed -E 's#^href="##; s#^https?://[^/]+##; s#^/##; s#"$##' | LC_ALL=C sort -u); do [ -f "$P/$href/index.html" ] || { echo "    homepage tile links to missing $href"; exit 1; }; done
