...
fn show_one(
    ...
    match format {
        OutputFormat::Json => {
            writeln!(out, "{}", pretty(&serde_json::to_value(node).unwrap_or(Value::Null)))
                .map_err(Error::Transport)?;
        }
        OutputFormat::Text => {
...
fn whole(args: &CommandsArgs, graph_args: &CommandsGraphArgs, format: OutputFormat) -> Result<u8> {
    ...
    match format {
        ...
        OutputFormat::Text => {
            ...
            writeln!(out, "fingerprint  {}", loaded.graph.fingerprint)
                .map_err(Error::Transport)?;
        ...
        match format {
            ...
            OutputFormat::Text => writeln!(
                ...
                if fresh {
                    ...
                    format!("stale; run `majordomus commands bridge` (graph {})", loaded.graph.fingerprint)
                }
            )
            .map_err(Error::Transport)?,
        }
