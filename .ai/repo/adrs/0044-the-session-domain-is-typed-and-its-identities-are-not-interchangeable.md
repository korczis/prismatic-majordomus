...
related:
  ...
  - file:apps/majordomus-cli/src/capability/builtin/session.rs
...
`Opening → Open`, `Open → Open` (resume and checkpoint, which do not move the state and
say so rather than being absent), `Open → Detached`, `Detached → Open`, `Open | Detached →
Closed`, and `Open | Detached → Closed` by recovery when the episode's last sign of life is
older than the stranded threshold. `Closed` is terminal: [`EpisodeState::may_move_to`]
answers `false` for every move out of it, which is the assertion the store makes before it
