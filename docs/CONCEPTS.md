...
| **knowledge record** | one assertion about the repository with a class, a status, an epistemic stance and provenance that resolves; a `candidate` is what the tool derived from the ledger and git at an episode boundary, `verified` is what a person promoted with evidence, and neither is ever a conversation | `.ai/repo/knowledge/candidates/<id>.md`, `.ai/repo/knowledge/curated/<id>.md`, `knowledge derive`, `knowledge promote`, `majordomus://knowledge/<id>` |
