...
sed -i.bak 's#^  - lib/auth$#  - lib/auth/#' .ai/local/state/current.yaml; rm -f .ai/local/state/current.yaml.bak
...
echo "## note" >> .ai/local/state/decisions.md; echo "" >> CLAUDE.md
expect_exit 0 "$MJ" check
...
sed -i.bak "s/\[unresolved\] $id/[resolved 2026-09-03] $id/" .ai/local/state/open-questions.md; rm -f .ai/local/state/open-questions.md.bak
expect_exit 0 "$MJ" check
...
stale() { sed 's/^checkpoint_at: .*/checkpoint_at: 2026-01-01T00:00:00Z/' .ai/local/state/current.yaml > "$T/.cur" && mv "$T/.cur" .ai/local/state/current.yaml; }
