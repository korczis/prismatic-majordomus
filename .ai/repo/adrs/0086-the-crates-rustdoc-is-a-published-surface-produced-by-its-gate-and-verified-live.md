...
| deployment | the tree composed from the topology, never committed | `scripts/site-build`, `scripts/site-deploy`, cases 486 and 489 |
| deployed verification | the public reference names the deployed commit and is whole | `scripts/pages verify-rustdoc`, the `pages-live` gate |
...
would sit on top of `/api/v1`, the capability routes; `/reference` already means three things
...
Library and executable, public items, rustdoc's own source pages kept. A reference built by a
different invocation from the one the gate checks is a second artifact nobody proved, so there
is one `cargo doc` line and case 77 fails a second. `--document-private-items` is not passed:
ADR 0028 decides what the public surface is, and publishing the private one would state the
opposite. The source pages are the source at exactly the commit the tree was built from, which
is the strongest source link a reference can carry.

### Not committed; generated in CI and proven fresh

The gate's own invocation wrote 81 MB in 2990 files when it was measured on 2026-09-24, at
`f7e05f4c5`, and the pages embed the values of the crate's `COMMIT` and `GENERATION`
constants — rustdoc renders `pub const COMMIT: &str = "<forty hex digits>";` on the constant's
page. Committed, the tree would be stale on every commit that touched the crate, and on every
commit at all through that one page, and would put its whole size into every clone. It is generated in CI instead: a
`rustdoc` job in `pages.yml` runs in parallel with the site's build and check, and the
validation workflow's site job obtains the tree through the same composite action,
`.github/actions/rustdoc`, so the two cannot build it differently.
...
so a site cannot be published missing a surface the topology says it carries.
...
exists without an item, that `built_from` is `HEAD`, that the crate index carries its identity
(`<title>majordomus_cli - Rust</title>`), that the index's assets are present, that internal
links resolve, and that no page names a machine path or a secret. It exits 0 clean, 10 with
findings and 12 when there is no tree to judge — never an empty pass. The inventory is not a
list kept for this consumer: it is the crate measuring itself, and the routes are derived from
it by the toolchain's own convention.
...
*`cargo doc` inside the deploy job.* It puts a Rust toolchain install and a documentation build
on the critical path of every publication, in series. A parallel job costs the difference
between the two paths instead of their sum, and the action it runs is the same one the
...
- **gh-pages grows by the tree once, then by its differences.** The first deployment adds the
  whole tree to the branch; later ones add what changed — the pages of items that changed, the
  constant pages, the search index. Unchanged pages are the same blobs, so git stores them once.
  `scripts/site-deploy` fetches the one branch it pushes, and that fetch grows with it.
- **Publication waits for the reference.** The deploy step needs the `rustdoc` job, so the time
  from push to public grows by however much longer that job runs than the site's own build and
  check. The job's cost is measured by the run that pays it, not estimated here.
...
- **The crate's sources can change the published bytes.** A change to the crate changes the
  reference, so the paths that start a publication include the crate's sources; they are still
  derived by `scripts/pages paths` from the gate model, never written into the workflow.
