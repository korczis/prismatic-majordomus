...
canonical_ids() { ls .ai/repo/project/issues/*.yaml | sed 's#.*/##; s#\.yaml$##' | LC_ALL=C sort; }
cli_ids()       { "$MJ" plan list | awk 'NR>1{print $1}' | LC_ALL=C sort; }
json_ids()      { "$MJ" --json plan list | tr ',' '\n' | sed -n 's/.*"id":"\([^"]*\)".*/\1/p' | LC_ALL=C sort; }
graph_ids()     { "$MJ" plan graph | sed -n 's/^    \([I0-9]*\)\[.*/\1/p' | LC_ALL=C sort; }
sync_ids()      { "$SYNC" --plan | awk '$2=="open" || $2=="closed" { print $1 }' | LC_ALL=C sort; }

cli_ready()   { "$MJ" plan ready | sed -n 's/^\(I[0-9]*\) .*/\1/p' | LC_ALL=C sort; }
list_ready()  { "$MJ" plan list | awk '$2=="READY"{print $1}' | LC_ALL=C sort; }
graph_ready() { "$MJ" plan graph | sed -n 's/^    \([I0-9]*\)\[.*:::ready$/\1/p' | LC_ALL=C sort; }
sync_open()   { "$SYNC" --plan | awk '/^  I[0-9]+ +open +READY/{print $1}' | LC_ALL=C sort; }
...
graph_edges()  { "$MJ" plan graph | sed -n 's/^    \(.* --> .*\)$/\1/p' | LC_ALL=C sort; }
...
PJ="$T/gen/plan.json"
  site_ids="$(jq -r '.issues[].id' "$PJ" | LC_ALL=C sort)"
  cli_real="$("$MJ" --repo "$ROOT" plan list | awk 'NR>1{print $1}' | LC_ALL=C sort)"
  same "generated site data and the CLI, same repository" "$site_ids" "$cli_real"
  site_ready="$(jq -r '.issues[] | select(.status=="READY") | .id' "$PJ" | LC_ALL=C sort)"
  cli_real_ready="$("$MJ" --repo "$ROOT" plan list | awk '$2=="READY"{print $1}' | LC_ALL=C sort)"
  same "generated site data and the CLI READY sets" "$site_ready" "$cli_real_ready"
  site_edges="$(jq -r '.edges[] | .from + " --> " + .to' "$PJ" | LC_ALL=C sort)"
  cli_real_edges="$("$MJ" --repo "$ROOT" plan graph | sed -n 's/^    \(.* --> .*\)$/\1/p' | LC_ALL=C sort)"
