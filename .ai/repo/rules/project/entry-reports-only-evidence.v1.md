---
id: project.entry-reports-only-evidence
version: 1
kind: rule
title: Entering the repository reports only what evidence proves
description: Repository bootstrap surfaces derive every status from the canonical preflight and never report a subsystem active, enforced or verified without the runtime or verification evidence behind it; the entry file and client bootstrap files stay thin consumers of that value.
statement: Repository bootstrap surfaces MUST derive status from canonical Majordomus state and MUST NOT report a subsystem as active, enforced or verified without corresponding runtime or verification evidence; `.envrc` and client bootstrap files MUST remain thin consumers of the canonical runtime and MUST NOT duplicate repository-domain logic.
status: active
class: blocking
depends_on: [project.envrc-is-an-adapter@2, project.entry-converges@2, project.interfaces-are-projections@1]
tags: [entry, environment, evidence, verification, enforcement]

x-majordomus:
  tests: [test/cases/358_entry_reports_only_evidence.sh, apps/majordomus-cli/tests/preflight.rs]
---
