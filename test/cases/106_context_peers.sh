...
command -v curl >/dev/null 2>&1 || skip_case "curl absent"
...
[ -x "$BIN" ] || skip_case "no built executable to read the lease with; the never-load-bearing half is proved above"
