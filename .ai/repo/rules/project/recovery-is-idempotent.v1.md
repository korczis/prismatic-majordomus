---
id: project.recovery-is-idempotent
version: 1
kind: rule
title: Recovery looks before it repeats
description: Before retrying interrupted work a worker establishes what already happened — artifacts on disk, git state, test and build output, task records — and resumes from there; recovery never duplicates a commit, a file, a migration or any other side effect.
