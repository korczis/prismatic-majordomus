...
[`GITHUB_PAGES_ARCHITECTURE.md`](@/docs/github-pages-architecture.md) says what the site *is* — the
canonical sources, the generators, the checks. This document is about the *path*: latency,
budgets and the evidence behind both. [`CI.md`](@/docs/ci.md) is the validation pipeline beside it.
[`PAGES_STATUS.md`](@/docs/pages-status.md) carries the current numbers, generated; nothing here is
...
- the crate's rustdoc reference is produced from this commit — it is part of the published
  bytes and is never committed ([`RUSTDOC.md`](@/docs/rustdoc.md)),
...
absent or names another commit ([`RUSTDOC.md`](@/docs/rustdoc.md)).
