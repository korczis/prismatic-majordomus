---
id: project.execution-state-is-authoritative
version: 1
kind: rule
title: Execution state is authoritative and the terminal is not
description: A worker decides whether to continue from execution state — exit status, structured output, state artifacts, messages, logs — and never from terminal focus, keystrokes or renderer activity; a finished delegate's result is consumed when it is ready, not when somebody looks at the pane.
statement: Forward progress is decided by execution state, never by terminal activity; a completed delegate's result is consumed as soon as it exists, and a dependency on focus, input or rendering is an implementation defect to be routed around.
status: active
class: advisory
