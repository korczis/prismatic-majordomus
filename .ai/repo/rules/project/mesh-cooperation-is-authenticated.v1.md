---
id: project.mesh-cooperation-is-authenticated
version: 1
kind: rule
title: Mesh cooperation is admitted per link, replicated through one journal, and projected by every surface alike
description: A runtime cooperates only over a link whose handshake proved the peer's key live, its repository identical, its protocol compatible and its key trusted; everything cooperative travels as signed events of the one journal, folds into one state, and reaches the CLI, HTTP, OpenAPI, MCP and the Cockpit through the mesh capability module alone.
statement: Discovery never admits a link; a link is admitted only by the handshake in mesh::cooperation — signature, freshness, nonce, protocol, self, repository identity, trust — and every refusal is typed and recorded; sessions, claims, handovers and reviews cross runtimes only as origin-signed events of mesh::journal::Journal, deduplicated by (stream, seq) and folded by mesh::state::fold, whose exclusivity rule is the same for a claim of this machine and of any other; liveness is a beat that expires on the local clock; and no surface holds peers, links, claims or connection state of its own.
status: active
class: blocking
depends_on: [project.mesh-is-observation-not-authority@1]
tags: [mesh, security, cooperation, doctrine]

x-majordomus:
  tests: [scripts/ci/mesh-check, apps/majordomus-cli/tests/mesh_cooperation.rs, test/mesh-lab/run]
---
