...
summary: 'Wire the provider''s own session events, and the episode opens when the sitting begins, closes when it ends, and leaves behind the context the worker was given at the open.'
category: continuity
status: active
target: guaranteed
actors: [agent, operator]
difficulty: basic
commands: [capture, session, doctor]
