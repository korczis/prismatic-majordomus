---
id: project.a-diagram-is-drawn-not-typed
version: 1
kind: rule
title: An architectural relationship in a published document is drawn, never typed in box art
description: A diagram made of box-drawing characters is a picture nothing can read — it cannot be rendered, linked, themed or checked against what it claims to describe, and on a phone it is the one element that cannot wrap. Every architectural relationship a published document states is a mermaid block; literal terminal output, file listings and quoted file content are not diagrams and are exempt by declaration.
statement: Draw an architectural relationship as a mermaid block in the one document that states it; box art is a terminal rendering, and a picture typed by hand is a projection nothing derives, nothing validates and nothing can redraw.
status: active
class: blocking
depends_on: [project.derived-once@1, project.interfaces-are-projections@1]
tags: [documentation, diagrams, web, accessibility]

x-majordomus:
  tests: [scripts/ci/diagram-check, test/cases/272_diagrams_are_drawn.sh]
---
