...
impl Selection {
    ...
    ///
    /// ```
    /// use majordomus_cli::capability::builtin::continuity::Selection;
    /// assert_eq!(Selection::Superseded.as_str(), "superseded");
    /// ```
    pub fn as_str(self) -> &'static str {
        match self {
...
impl CitationKind {
    ...
    ///
    /// ```
    /// use majordomus_cli::capability::builtin::continuity::CitationKind;
    /// assert_eq!(CitationKind::LedgerLine.as_str(), "ledger_line");
    /// assert_eq!(CitationKind::Absent.as_str(), "absent");
    /// ```
    pub fn as_str(self) -> &'static str {
        match self {
            CitationKind::File => "file",
            CitationKind::Symlink => "symlink",
            CitationKind::LedgerLine => "ledger_line",
            CitationKind::PolicyKey => "policy_key",
            CitationKind::Absent => "absent",
        }
    }
}

/// One thing a reader can go and check: the evidence behind a single claim.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize, JsonSchema)]
pub struct Citation {
...
impl Citation {
    ...
    ///
    /// ```
    /// use majordomus_cli::capability::builtin::continuity::{Citation, CitationKind};
    /// let e = Citation::absent(".ai/local/prompts", "the directory does not exist");
    /// assert_eq!(e.kind, CitationKind::Absent);
    /// ```
    pub fn absent(locator: impl Into<String>, detail: impl Into<String>) -> Self {
        Citation {
            kind: CitationKind::Absent,
            locator: locator.into(),
            detail: detail.into(),
        }
    }
...
pub struct Answer {
    ...
    /// [`CitationKind::Absent`] rather than as an empty list, so that a sentence with no
...
impl Answer {
    ...
    fn unknown(question: &str, reason: impl Into<String>, evidence: Vec<Citation>) -> Self {
        Answer {
            question: question.into(),
            answer: String::new(),
            known: false,
    ...
    fn weighing(mut self, considered: Vec<Considered>) -> Self {
...
pub struct ExplainEpisodeInput {
...
impl BenchmarkCases for ExplainEpisodeInput {
...
/// read as records, and one [`Considered`] entry per `.md` file the directory held — the
...
fn resolve(
    ...
    now: i64,
) -> (Option<Record>, usize, Vec<Considered>) {
    ...
    let mut trace: Vec<Considered> = Vec::new();
    ...
    for path in paths {
        ...
        let mut reject = |reason: String| {
            trace.push(Considered {
                path: rel.clone(),
                outcome: Selection::Rejected,
                reason,
            });
        };
...
fn q_episode_current(
    ...
    match &target {
        Some(t) => evidence.push(Citation {
            kind: CitationKind::Symlink,
            locator: rel_pointer.clone(),
            detail: format!("-> {t}"),
        }),
        None if pointer.exists() => evidence.push(Citation::file(
            &rel_pointer,
        ...
        None => evidence.push(Citation::absent(
            &rel_pointer,
...
fn q_episode_close(subject: &Subject, lines: &[LedgerLine], closed: Option<&BTreeMap<String, String>>) -> Answer {
    ...
    if subject.episode.is_empty() {
        return without_an_episode(Q, "Nothing was searched for in the ledger.");
    }
    ...
    if subject.state == "open" {
        ...
        for l in &closes {
            evidence.push(Citation::ledger(l.number, l.summary()));
        }
        return Answer::known(
            Q,
            "not closed",
...
fn q_episode_task(
    ...
    if subject.episode.is_empty() {
        return without_an_episode(Q, "No record and no ledger line was consulted.");
    }
...
/// The answer the four episode questions give when no episode was resolved at all.
///
/// One helper rather than four sentences, because they are one fact: there is no subject.
/// Each of them used to reach for `subject.record` as the thing to cite, which is the empty
/// string in exactly this case — and a citation with no locator is a sentence with nothing
/// behind it, which is the defect this whole module exists to refuse. `test/cases/270`
/// caught it; the unit test beside it now checks every locator, not merely that a list is
/// non-empty.
fn without_an_episode(question: &str, looked_at: &str) -> Answer {
    Answer::unknown(
        question,
        format!(
            "this checkout resolved no episode, so there is no subject for this question; \
             see episode.current for why. {looked_at}"
        ),
        vec![Citation::absent(
            format!("{STATE_DIR}/session-current.yaml"),
            "no episode was resolved, so nothing could be looked up by its id",
        )],
    )
}
...
fn q_episode_prompts(root: &Path, subject: &Subject) -> Answer {
    ...
    let Ok(entries) = std::fs::read_dir(&dir) else {
        return Answer::unknown(
            ...
            vec![Citation::absent(REL, "the directory does not exist")],
        );
    };
    ...
    for rel in by_episode.iter().take(3) {
        evidence.push(Citation::file(rel, "episode recorded on the record itself"));
    }
    ...
    if subject.episode.is_empty() {
        let mut a = without_an_episode(Q, "The archive was read and nothing was joined to it.");
        // The archive's own totals are still worth having even with no episode to join
        // them to: "1017 records, 0 unreadable" is a fact about this checkout.
...
fn q_episode_recovery(subject: &Subject, lines: &[LedgerLine]) -> Answer {
    ...
    if subject.episode.is_empty() {
        return without_an_episode(Q, "No ledger line was matched against one.");
    }
    ...
    let evidence: Vec<Citation> = hits
        .iter()
        .map(|l| {
            Citation::ledger(
                l.number,
...
fn q_selected(kind: &str, record: Option<&Record>, trace: Vec<Considered>, dir_rel: &str) -> Answer {
    let q = format!("{kind}.selected");
    match record {
        Some(r) => {
            let selected = trace
                .iter()
                .find(|c| c.outcome == Selection::Selected)
...
fn q_freshness(
    ...
    match (thresholds.fresh_minutes, thresholds.stale_minutes) {
        ...
        _ => evidence.push(Citation::absent(
            policy_path,
...
fn explain(ctx: &Context, input: ExplainEpisodeInput) -> Result<SessionProvenance, CapabilityError> {
    let root = PathBuf::from(&ctx.index.repository.root);
    let dir = root.join(STATE_DIR);
...
/// it came from is the whole point here: "stale past two days" is an assertion, and
/// "`.ai/repo/policy.yaml#session.freshness.stale_minutes` says so, and here is what it
/// says" is a fact. The number is never written down on this side of the read — including
/// in this sentence, which would otherwise be the first thing to go stale when the policy
/// moved.
...
mod tests {
    ...
    fn the_resolution_rule_reports_why_each_candidate_lost() {
        ...
        assert_eq!(by("b.md").outcome, Selection::Selected);
        assert_eq!(by("a.md").outcome, Selection::Superseded);
        assert!(
        ...
        assert_eq!(by("c.md").outcome, Selection::Rejected);
    ...
    fn a_record_from_another_branch_is_rejected_with_the_branch_named() {
        ...
        assert_eq!(trace[0].outcome, Selection::Rejected);
        assert!(
    ...
    fn every_answer_carries_a_reason_and_something_to_check() {
        ...
        let out = explain(&ctx, ExplainEpisodeInput::default()).expect("an explanation");
        ...
        for a in &out.answers {
            ...
            // And every citation names somewhere. This assertion is here because its
            // absence let `episode.task` ship a citation whose locator was the empty
            // string — `subject.record` for a checkout with no episode — and a list being
            // non-empty said nothing about whether a reader could follow any of it.
            for e in &a.evidence {
                assert!(
                    !e.locator.is_empty(),
                    "{} cites something with no location",
                    a.question
                );
                assert!(
                    !e.detail.is_empty(),
                    "{} cites {} and says nothing about it",
                    a.question,
                    e.locator
                );
            }
            // `answer` is empty exactly when `known` is false. An unknown that carries a
            // likely-looking string is the invention this module exists to refuse, and a
            // known answer that says nothing is a verdict with no content.
            assert_eq!(
                a.answer.is_empty(),
                !a.known,
                "{} has known={} and answer={:?}",
                a.question,
                a.known,
                a.answer
            );
    ...
    fn a_checkout_with_no_open_episode_says_so_rather_than_inventing_one() {
        ...
        assert_eq!(current.evidence[0].kind, CitationKind::Absent);
    ...
    fn the_verdict_names_the_threshold_it_crossed_and_the_file_that_declares_it() {
        ...
        assert!(
            a.evidence.iter().any(|e| e.kind == CitationKind::PolicyKey),
    ...
    fn a_record_with_no_thresholds_to_judge_it_by_is_unjudged_rather_than_fresh() {
        ...
        assert_eq!(a.evidence[0].kind, CitationKind::Absent);
    }
    ...
    fn a_question_can_be_narrowed_by_name_or_by_prefix() {
        let repo = with_state(&[]);
        let ctx = repo.context().expect("a context");
        let all = explain(&ctx, ExplainEpisodeInput::default()).expect("every question");
        let one = explain(
            &ctx,
            ExplainEpisodeInput {
                episode: None,
