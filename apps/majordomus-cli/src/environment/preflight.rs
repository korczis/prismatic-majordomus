...
///     derive, GateRecordObservation, GitObservation, Observations, Verdict, COVERAGE_GATE,
/// };
/// let mut o = Observations::empty("demo", 0);
/// o.git = Some(GitObservation { branch: None, head: Some("a".repeat(40)), clean: true, changed: 0 });
...
/// A check over a gate's recorded run: `verified` when it passed over the clean tree in
/// front of you, `failed` when it failed there, `stale` when it measured another tree or the
/// tree is dirty, `unknown` when the tree could not be hashed. `absent` is the check when nothing is recorded.
fn recorded_gate_check(
    ...
    match run.current {
        ...
        .next(remediation),
        // The line records no tree state, so a run whose inputs match a dirty tree may have
        // measured uncommitted work: at best stale, never verified. Over a clean tree the
        // matching inputs are the committed ones, which is what a verification proves.
        Some(true) if o.git.as_ref().map(|g| g.clean) != Some(true) => Check::new(
            id,
            title,
            Verdict::Stale,
            format!(
                "{gate} passed over these inputs, and the tree is not clean, so it proves no commit{said}"
            ),
            evidence,
        )
        .next(remediation),
