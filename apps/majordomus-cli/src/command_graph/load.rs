...
/// exposure is declared by a builtin module, so the join is complete without one.
fn registry() -> Option<CapabilityRegistry> {
...
pub fn write_cache(root: &Path, graph: &CommandGraph) -> std::io::Result<()> {
    ...
    std::fs::write(&tmp, serde_json::to_vec(graph).map_err(std::io::Error::other)?)?;
