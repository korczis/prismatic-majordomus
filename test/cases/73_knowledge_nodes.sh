...
. "$ROOT/test/lib.sh"
# The unknown-kind case mutates the source list. Shared classes are declared by the
# repository's own AI layer, so the mutation goes into this disposable repository's copy and
# never near the tool's shipped operational classes.
SRC=".ai/repo/knowledge/sources.yaml"
...
expect_grep '^policy +shared +[0-9a-f]{12} +policy:\.ai/repo/policy\.yaml' a.txt
...
other_before="$(awk '$4 == "policy:.ai/repo/policy.yaml" { print $3 }' a.txt)"
other_after="$(awk '$4 == "policy:.ai/repo/policy.yaml" { print $3 }' c.txt)"
...
expect_grep 'FAIL +knowledge +policy:\.ai/repo/policy\.yaml — claimed by'
