...
//! worktree, which is no lock at all. It is not under `/tmp` or the home directory either,
//! which would be machine-wide.
...
/// the lock and releasing it must not stop the repository for ever. Generous, because the
/// operations under it are `git worktree add` and `git worktree move`, which are fast.
...
mod tests {
    ...
    fn a_second_holder_waits_and_then_names_the_first() {
        ...
        drop(held);
        // and the lock is free again the moment the holder is dropped
