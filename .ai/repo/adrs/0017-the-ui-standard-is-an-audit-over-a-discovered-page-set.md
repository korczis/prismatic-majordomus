...
its job. That exemption was blind to the application having *files* at particular paths. The
validator now asks the application's output whether it has pages under a native route's mount,
...
The `/docs` shadowing is now reported by `web validate` with the page count, rather than
