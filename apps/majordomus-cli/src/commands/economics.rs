...
use crate::cli::{
    EconomicsArgs, EconomicsCheckArgs, EconomicsCommand, EconomicsExplainArgs, EconomicsMeasureArgs,
    EconomicsReferencesArgs, EconomicsRunArgs, EconomicsRunsArgs, EconomicsSummaryArgs,
...
fn map(e: CapabilityError) -> Error {
    match e {
        CapabilityError::InvalidInput(reason) | CapabilityError::Refused(reason) => Error::Refused {
...
fn print_json(value: &Value) -> Result<()> {
    ...
    writeln!(out, "{}", serde_json::to_string_pretty(value).unwrap_or_else(|_| value.to_string()))
...
fn render_summary(out: &mut impl Write, s: &EconomicsSummary) -> std::io::Result<()> {
    writeln!(out, "Token economics{}", s.methodology.map(|m| format!(" (methodology {m})")).unwrap_or_default())?;
    ...
    if !s.segments.is_empty() {
        ...
        for g in &s.segments {
            let med = g.token_reduction.as_ref().map(|d| percent_text(d.median)).unwrap_or_else(|| "—".into());
            writeln!(out, "  {:<11} {:<20} {:>8}  n={}", g.dimension, g.value, med, g.n)?;
        }
    }
...
fn explain(a: EconomicsExplainArgs) -> Result<u8> {
    let value = ask(&a.repo, &["economics", "explain"], json!({ "metric": a.metric }))?;
    ...
    let w = |out: &mut std::io::StdoutLock, s: String| writeln!(out, "{s}").map_err(Error::Transport);
    w(&mut out, format!("{}  —  {}", m.id, m.title))?;
    w(&mut out, format!("value        {}   ({})", value_text(m), words(&m.status)))?;
    w(&mut out, format!("class        {}: {}", class_text(m), e.class_meaning))?;
    w(&mut out, format!("formula      {}", m.formula))?;
    if let Some(not) = &m.not {
        w(&mut out, format!("not          {not}"))?;
    }
    w(&mut out, format!("sample       n={} (methodology {})", m.n, e.methodology))?;
    if let Some(d) = &m.distribution {
        let a = |v: f64| amount(v, &m.unit);
        w(&mut out, format!(
            "distribution median {}  mean {}  p25 {}  p75 {}  p95 {}  min {}  max {}{}",
            a(d.median), a(d.mean), a(d.p25), a(d.p75), a(d.p95), a(d.min), a(d.max),
            d.sd.map(|x| format!("  sd {}", spread_text(x, &m.unit))).unwrap_or_default()
        ))?;
    }
    if let Some(i) = &m.interval {
        w(&mut out, format!(
            "interval     {} ({}; {} resamples, seed {})",
            interval_text(i, &m.unit), i.method, i.resamples, i.seed
        ))?;
    }
    for s in &e.suites {
        w(&mut out, format!(
            "suite        {} v{} ({}), {} run(s), freshness {}, revisions {}",
            s.id, s.version, s.kind, s.runs, words(&s.freshness), s.revisions.join(", ")
        ))?;
    }
    for p in &e.pairs {
        w(&mut out, format!(
            "pair         {}/{}/r{}  {}  {}{}",
            p.suite, p.task, p.repetition, words(&p.status),
            p.token_reduction.map(|r| format!("token reduction {}", percent_text(r))).unwrap_or_default(),
            if p.reasons.is_empty() { String::new() } else { format!("  ({})", p.reasons.join("; ")) }
        ))?;
    }
    if evidence_is_seeds(m) {
        w(&mut out, format!("evidence     {} seed(s): {}", m.evidence.len(), m.evidence.iter().take(12).cloned().collect::<Vec<_>>().join(", ")))?;
    }
    for x in &e.excluded {
...
fn runs(a: EconomicsRunsArgs) -> Result<u8> {
    ...
    for r in &l.runs {
        ...
        writeln!(
            ...
            u.map(|u| u.tool_calls.to_string()).unwrap_or_else(|| "—".into()),
            r.path
        )
        .map_err(Error::Transport)?;
    }
...
fn declarations(root: &Path) -> Result<economics::Declarations> {
    match economics::load(root) {
        ...
        Ok(None) => Err(Error::NotFound {
            reason: format!("no benchmark methodology is declared: {}/methodology.yaml does not exist", economics::DIR),
        }),
        Err(e) => Err(Error::Refused {
            code: EXIT_FINDINGS,
            reason: format!("the benchmark declarations do not read:\n  {}", e.join("\n  ")),
        }),
    }
}
...
fn measure(a: EconomicsMeasureArgs) -> Result<u8> {
    ...
    let suite = decl.suites.get(&a.suite).ok_or_else(|| Error::NotFound {
        reason: format!("no suite {:?}; the suites are: {}", a.suite, decl.suites.keys().cloned().collect::<Vec<_>>().join(", ")),
    })?;
    if suite.kind != "context" {
        return Err(Error::Refused {
            code: crate::cli::EXIT_USAGE,
            reason: format!("suite {} is a {} suite: `majordomus economics run --suite {}` runs it", suite.id, suite.kind, suite.id),
        });
    }
    let at = crate::peers::rfc3339(SystemTime::now());
    let (record, errors) = economics::context::measure(&app.context, &decl, suite, &at)
        .map_err(|reason| Error::Refused { code: EXIT_FINDINGS, reason })?;
    ...
    let path = root.join(economics::DIR).join("runs").join(&suite.id).join(format!("{short}.json"));
    let mut out = std::io::stdout().lock();
    writeln!(
        out,
        ...
        suite.id,
        record.seeds.len(),
        if record.repository.dirty { " (dirty)" } else { "" },
        totals.0,
        totals.1,
    ...
    .map_err(Error::Transport)?;
    if a.dry_run {
        writeln!(out, "dry run: nothing written (would write {})", path.strip_prefix(&root).unwrap_or(&path).display()).map_err(Error::Transport)?;
    } else {
        std::fs::create_dir_all(path.parent().unwrap_or(&root)).map_err(Error::Transport)?;
        let mut text = serde_json::to_string_pretty(&record).map_err(|e| Error::Protocol { reason: e.to_string() })?;
        text.push('\n');
        std::fs::write(&path, text).map_err(Error::Transport)?;
        writeln!(out, "recorded {}; read it with: majordomus economics summary", path.strip_prefix(&root).unwrap_or(&path).display()).map_err(Error::Transport)?;
    }
    Ok(if errors.is_empty() { 0 } else { EXIT_FINDINGS })
}

fn default_work_dir(suite: &str) -> PathBuf {
    std::env::temp_dir().join("majordomus-economics").join(suite)
}
...
fn live(a: EconomicsRunArgs) -> Result<u8> {
    ...
    if !majordomus.is_file() {
        return Err(Error::NotFound {
            reason: format!("the treatment installs Majordomus from {}, which does not exist", majordomus.display()),
        });
    }
    ...
    let log = |line: String| {
        let _ = writeln!(std::io::stderr().lock(), "{} {line}", crate::peers::rfc3339(SystemTime::now()));
    };
    let (written, failed) = runner::run(&opts, &decl, &log).map_err(|reason| Error::Refused { code: crate::cli::EXIT_USAGE, reason })?;
    let mut out = std::io::stdout().lock();
    for p in &written {
        writeln!(out, "recorded {}", p.strip_prefix(&root).unwrap_or(p).display()).map_err(Error::Transport)?;
    }
    let failed: Vec<&String> = failed.iter().filter(|f| !(a.dry_run && f.ends_with("dry run"))).collect();
    for f in &failed {
        writeln!(out, "FAIL {f}").map_err(Error::Transport)?;
    }
    writeln!(out, "{} run(s) recorded, {} failed to produce a record", written.len(), failed.len()).map_err(Error::Transport)?;
