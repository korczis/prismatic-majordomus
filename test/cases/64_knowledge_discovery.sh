...
expect_grep '^policy +shared +policy +[0-9a-f]{12} +\.ai/repo/policy\.yaml$' a.txt

# --- editing a source changes that source's hash and no other
before="$(awk '$5==".ai/repo/policy.yaml"{print $4}' a.txt)"
other_before="$(awk '$5=="docs/README.md"{print $4}' a.txt)"
printf '\n# a comment the parser ignores\n' >> .ai/repo/policy.yaml
"$MJ" knowledge sources > d.txt
after="$(awk '$5==".ai/repo/policy.yaml"{print $4}' d.txt)"
...
git checkout -- .ai/repo/policy.yaml 2>/dev/null || true
...
expect_no_grep '\.ai/local/state/' shared.txt
expect_no_grep ' operational ' shared.txt
grep -q '\.ai/local/state/checkpoints/' op.txt || { echo "    an operational record was not discovered"; exit 1; }
