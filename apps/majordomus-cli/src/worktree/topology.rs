...
//! alignment in it. Everything downstream — the policy evaluation, the doctor check, the
//! selectors, the migration plan — reads these records; nothing anywhere else runs
//! `git worktree list` or looks at its human output.
...
impl WorktreeRecord {
    /// The directory name, which is a worktree's short selector.
