---
id: project.a-tool-that-cannot-match-the-tree-refuses
version: 1
kind: rule
title: A generator that cannot match the tree refuses rather than rewriting it
description: An executable asked to generate this repository's derived artifacts first establishes that it was built from the same declarations the tree carries; when it cannot, it refuses and says so, because a generator that guesses rewrites the documents it was meant to check.
statement: A generator whose build does not match the tree it is asked to derive refuses, names both sides, and exits non-zero; it never writes.
status: active
class: blocking
