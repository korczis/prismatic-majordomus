---
id: project.executions-carry-no-secret
version: 1
kind: rule
title: A sensitive value never enters an execution
description: A value an input schema marks sensitive is replaced once, before the input is stored, so that no execution snapshot, event, history, log, page, listing or generated document can carry it; what a schema marks is the only rule, and a field's name is never one.
statement: Redact from the schema, once, before the value is stored; never from a field's name, never a second time in a transport, and never in a page.
status: active
class: blocking
