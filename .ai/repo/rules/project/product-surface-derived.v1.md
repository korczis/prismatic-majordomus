---
id: project.product-surface-derived
version: 1
kind: rule
title: What the product does is a catalogue object, and every public surface of it is derived
description: Every product feature is one schema-valid file under the layer's features section, holding references and editorial decisions only; the interfaces, counts, links, moments and routes every surface shows are derived from those references, and a page, template, generator or navigation file that names a feature, module, command, provider or count by hand is a defect.
statement: A product feature is authored once as an object of the layer, with references and editorial decisions only; every fact a surface states about it is derived, and naming a feature, module, command, provider or count in a template, a generator or a navigation file is a defect.
status: active
class: blocking
