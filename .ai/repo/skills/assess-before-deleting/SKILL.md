...
status: active
tags: [cleanup, safety, evidence, worktrees]
related: [implement, report-verification-state]
inputs:
  - the candidates for removal, each named by its path, ref, record id or process id
  - a checkout with git history, the peer board and the repository's own diagnostics
outputs:
  - a removal table naming each candidate, its class, the evidence behind the class, its risk and the action taken
  - a rescue ref or pushed branch for anything that held work not reachable elsewhere
  - the verification that ran after the removal, with its exit codes
provenance:
  origin: prior-art
  ledger: import-2026-09-09#2
  decision: adapted
---
