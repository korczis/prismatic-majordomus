---
id: project.operation-transport-parity
version: 1
kind: rule
title: A command is the projection of a capability, or it says why it is not, and the reason is checked
description: Every runnable command of the native command line is bound to a canonical capability, or classified in one typed place with a structural reason that the parity check verifies; a command in neither place is a finding, and so is a classification that has gone stale.
statement: An operation absent from the API is absent because somebody decided it, in a place that is checked; absence by nobody noticing is a defect.
status: active
class: blocking
