...
| [`COMMANDS.md`](COMMANDS.md) | everyone, AI workers | the canonical command graph: what is declared once, how the `just` bridge, shell completion, MCP and the documentation are computed from it, how exposure follows the effect classification, and how to add a command or a workflow |
