...
description: What this project has shipped is derived from the layer's own release records, the decisions dated inside each release's window and the commits in its range, so no changelog is kept by hand; the version is authored in exactly one place, the crate manifest, projected once for the shell tool, recorded by the release pipeline, and written by one command, so there is no second statement of it anybody could forget to change.
statement: Compose the changelog from the records, the decisions and the commits the repository already holds, and author the version only in the crate manifest's `[package] version`, raised by the one writer; every other statement of it is derived — compiled, generated or recorded — and a version anybody writes down by hand elsewhere is a second declaration of a fact this repository already states.
status: active
class: blocking
...
x-majordomus:
  tests: [scripts/ci/release-check, test/cases/103_release_projection.sh, test/cases/484_the_version_is_authored_once.sh]
---
