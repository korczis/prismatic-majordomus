...
status = "guaranteed"
source = "docs/claims/mcp-client-autostart.md"
+++
{% raw %}
...
`bin/majordomus-mcp` (portable bash) resolves the repository from its own location, picks `apps/majordomus-cli/target/<profile>/majordomus` (`MAJORDOMUS_BUILD_PROFILE`, default `debug`), compares it with the sources with `find -newer`, runs `cargo build --locked` when needed with the output on stderr, sets `MAJORDOMUS_SHARE` to the distribution's `share/` beside it, and `exec`s the executable with `mcp` and every argument it was given. One argument it keeps: a leading `--provider <id>` is entry — the launcher runs `capture session --event start --launcher` on that provider's behalf before the protocol starts, which is how a client that fires no start event of its own still ensures the server and resumes this checkout's episode (`docs/ENTRY.md`). It never writes to stdout, which belongs to the protocol; the briefing goes to stderr. `MAJORDOMUS_BIN` names an executable to use without building; `MAJORDOMUS_NO_BUILD=1` refuses to build and exits 12 with the command to run. `.mcp.json` (Claude Code, project scope, approved once), `.gemini/settings.json` (Gemini CLI, `mcpServers.majordomus`) and `.codex/config.toml` (Codex, `[mcp_servers.majordomus]`, loaded when the project is trusted) all name it.
