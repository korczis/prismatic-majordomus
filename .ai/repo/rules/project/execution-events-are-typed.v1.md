---
id: project.execution-events-are-typed
version: 1
kind: rule
title: A long operation reports typed events, and a transport only renders them
description: An operation long enough to be watched reports its steps, its progress and its findings as typed execution events produced by the domain and published to one store; the WebSocket, the HTTP history route, the command line and the Cockpit render that value and contain no execution logic, and the schema every client validates against is derived from the Rust types rather than written down.
statement: Report progress as typed execution events from the domain handler, publish them once, and let every transport render them; a transport that decides what an execution is, when it may change state, or what an event means is a bug, and so is a stream of untyped lines.
status: active
class: blocking
