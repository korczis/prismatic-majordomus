...
pub(crate) struct Pattern {
    ...
    pub name: String,
    /// The expression as written, compiled.
    regex: Regex,
}
    ...
    pub(crate) fn scan(&self, text: &[u8], classes: &[LeakClass]) -> Vec<Leak> {
        ...
        for p in self.patterns.iter().filter(|p| classes.contains(&p.class)) {
            ...
            for (n, line) in text.split(|b| *b == b'\n').enumerate() {
                let hit = p.regex.find_iter(line).find(|m| {
                    p.class != LeakClass::Credential || !self.not_credential(m.as_bytes())
                });
                if let Some(m) = hit {
                    out.push(Leak {
                        class: p.class,
...
mod tests {
    ...
    fn the_shipped_definition_compiles_and_names_every_class_a_reader_asks_for() {
        ...
        assert!(leaks
            .patterns()
            .iter()
            .all(|p| DEFINITION.contains(&format!("\t{}\t{}", p.name, p.regex.as_str()))));
    }
