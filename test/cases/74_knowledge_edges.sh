...
cat >> .ai/repo/knowledge/sources.yaml <<'SRC'
  ...
  - id: case
    ...
    required: false
SRC
mkdir -p docs .ai/repo/project/milestones .ai/repo/project/issues
...
EOF
cat > .ai/repo/project/milestones/M100.yaml <<'EOF'
id: M100
title: An outcome
slug: an-outcome
order: 0
...
EOF
for n in 1 2; do
  cat > ".ai/repo/project/issues/I010$n.yaml" <<EOF
id: I010$n
milestone: M100
title: Issue $n
slug: issue-$n
...
# --- a rule's dependency is an edge, stated in its front matter as id@version; the node is
#     the rule's identity, and the provenance is the line that stated it. init seeded the
#     vendored baseline, and discovery reads the index, so this waits for the commit.
"$MJ" knowledge edges > rules.txt
expect_grep '^depends_on +rule:majordomus\.roadmap-integrity +rule:majordomus\.define-done-first +\.ai/repo/rules/vendor/majordomus/rules/roadmap-integrity\.v1\.md:depends_on\.0$' rules.txt
expect_no_grep 'rule:majordomus\.define-done-first@' rules.txt
"$MJ" knowledge nodes --kind rule > rnodes.txt
expect_grep '^rule +shared +[0-9a-f]{12} +rule:majordomus\.roadmap-integrity +The roadmap is a projection' rnodes.txt
expect_no_grep 'unknown_kind|has no rule for' rnodes.txt
rm -f rules.txt rnodes.txt
...
expect_grep '^part_of +issue:I0101 +milestone:M100 +\.ai/repo/project/issues/I0101\.yaml:milestone$' e.txt
expect_grep '^depends_on +issue:I0102 +issue:I0101 +\.ai/repo/project/issues/I0102\.yaml:depends_on\.0$' e.txt
expect_grep '^declares +milestone:M100 +claim:a-claim +\.ai/repo/project/milestones/M100\.yaml:claims\.0$' e.txt
...
expect_grep '"from":"issue:I0101","to":"milestone:M100","type":"part_of","provenance":"\.ai/repo/project/issues/I0101\.yaml:milestone"'
