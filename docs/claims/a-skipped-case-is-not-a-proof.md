# A test case that declines to run is reported as a skip and recorded as one, so the evidence ledger can never read a case that asserted nothing as the proof of the claim it names
...
A skip now has its own word from end to end: the case exits 4, the runner prints `skip` and
writes `SKIP`, and the ledger records `skip` — an outcome that does not prove. It is still
not a failure: the run's exit status does not change, and a run whose cases all declined is
a run that happened rather than "no cases found".
...
collapsing every non-zero to 1, and then maps it: 0 is a pass, 4 is a skip, everything else
is a failure. The runner's own statuses — 2 for a fixture it could not set up, 3 for the
bound firing — are returned by the runner itself and are never read from the case, so a
case that exits 3 of its own accord is a failed case and not a reported timeout.
...
the three words in both phases. It then records `SKIP` for a guaranteed claim in a fixture
