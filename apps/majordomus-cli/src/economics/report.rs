...
pub fn interval_text(i: &EconomicsInterval, unit: &str) -> String {
    format!("{}: {} to {}", level_text(i.level_bp), amount(i.low, unit), amount(i.high, unit))
}
...
pub fn markdown(s: &EconomicsSummary) -> String {
    ...
    for v in &s.variants {
        let _ = writeln!(o, "- **{}** ({}, `{}`): {}", v.title, v.role, v.id, v.description);
    ...
    for v in &s.suites {
        let _ = writeln!(
            ...
            v.pairs.both_failed,
            v.pairs.other,
            v.revisions.iter().map(|r| format!("`{}`", &r[..r.len().min(12)])).collect::<Vec<_>>().join(" "),
            v.harnesses.join(", "),
    ...
    for v in s.suites.iter().filter(|v| v.freshness_detail.is_some()) {
        let _ = writeln!(o, "- `{}`: {}", v.id, v.freshness_detail.as_deref().unwrap_or(""));
    }
    let _ = writeln!(o, "\n## Metrics\n");
    let _ = writeln!(o, "| metric | value | status | class | n | interval | what it is not |");
    let _ = writeln!(o, "|---|---|---|---|---|---|---|");
    for m in &s.metrics {
    ...
    for m in &s.metrics {
        ...
        if let Some(i) = &m.interval {
            let _ = writeln!(o, "  - interval: {}; {} resamples, seed {}.", i.method, i.resamples, i.seed);
        }
        for w in &m.warnings {
            let _ = writeln!(o, "  - {w}");
        }
    ...
    if !s.segments.is_empty() {
        ...
        let _ = writeln!(o, "| dimension | segment | valid pairs | median reduction | lowest | highest |");
        let _ = writeln!(o, "|---|---|---|---|---|---|");
        for g in &s.segments {
    ...
    if !live.is_empty() {
        ...
        for p in live {
            let _ = writeln!(
                ...
                p.repetition,
                words(&p.status),
                p.control_usage.as_ref().map(|u| u.total.to_string()).unwrap_or_else(|| "—".into()),
                p.treatment_usage.as_ref().map(|u| u.total.to_string()).unwrap_or_else(|| "—".into()),
                p.token_reduction.map(percent_text).unwrap_or_else(|| "—".into()),
                p.cost_reduction.map(percent_text).unwrap_or_else(|| "—".into()),
        ...
        let valid = s.pairs.iter().filter(|p| p.status == EconomicsPairStatus::Valid).count();
        let _ = writeln!(o, "{valid} of {} declared pair(s) are valid.\n", s.pairs.len());
    }
    ...
    if !s.history.is_empty() {
        ...
        let _ = writeln!(o, "| suite | methodology | revision | recorded | metric | value | n |");
        let _ = writeln!(o, "|---|---|---|---|---|---|---|");
        for h in &s.history {
            let _ = writeln!(o, "| {} | {} | `{}` | {} | `{}` | {} | {} |", h.suite, h.methodology, &h.revision[..h.revision.len().min(12)], h.at, h.metric, percent_text(h.value), h.n);
        }
        let _ = writeln!(o);
    }
    ...
    let _ = writeln!(o, "majordomus economics measure            # the context suite; deterministic, no model");
    let _ = writeln!(o, "majordomus economics run --suite pilot  # live sessions; needs a Claude Code login and spends usage");
    let _ = writeln!(o, "majordomus economics summary            # recompute every figure from the recorded runs");
    ...
    let _ = writeln!(o, "- Cost is the harness's own projection at run time, not a bill.");
    let _ = writeln!(o, "- Context reduction is measured against what the compiler itself judged relevant.");
...
mod tests {
    ...
    fn a_ratio_is_a_signed_percentage_with_one_decimal() {
        ...
        assert_eq!(value(&metric("ratio", Some(-0.0004))), "0.0%", "never -0.0%");
    }
    ...
    fn a_pair_shows_its_reduction_with_the_sign_the_calculator_gave_it() {
        let text = markdown(&summary(true, vec![valid(-0.1), valid(0.25), valid(-0.0001)]));
        assert!(text.contains("| token reduction | cost reduction |"), "{text}");
        assert!(!text.contains("token change"), "{text}");
        let rows: Vec<&str> = text.lines().filter(|l| l.starts_with("| vat-rounding |")).collect();
        assert_eq!(rows.len(), 3);
        assert!(rows[0].contains("| valid | — | — | -10.0% | -10.0% | 0 |"), "{}", rows[0]);
    ...
    fn a_state_is_spelt_as_it_serialises_and_an_interval_at_its_declared_level() {
        ...
        let mut s = summary(true, vec![EconomicsPair { status: EconomicsPairStatus::ControlFailed, ..pair(&[]) }]);
        ...
        assert!(text.contains("  - interval: cluster bootstrap over tasks; 2000 resamples, seed 7."));
    ...
    fn records_that_could_not_be_read_are_listed_even_without_a_methodology() {
        ...
        s.diagnostics.clear();
        assert!(!markdown(&s).contains("could not be read"), "no problem, no section");
    }
    ...
    fn the_limitations_name_the_shell_only_bootstrap_and_the_recorded_revision() {
        ...
        assert!(limits.contains("at the revision it recorded, not at the current one"), "{limits}");
    }
