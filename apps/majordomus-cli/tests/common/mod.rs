...
/// One deployment, so every capability the shipped registry declares has a case to time.
/// `deploy.get` reads an object by its id, and a fixture with no deployment leaves that
/// target uncovered — which the coverage gate reports and `bench baseline update` refuses.
pub const DEPLOYMENT: &str = "schema: deployment/v1
kind: deployment
id: fixture
title: The fixture deployment
description: One machine, declared so the deployment capabilities have something to read.
status: declared
...
impl Fixture {
    ...
    pub fn new() -> Self {
        ...
        f.write(".ai/repo/deployments/fixture.yaml", DEPLOYMENT);
        f.write(
