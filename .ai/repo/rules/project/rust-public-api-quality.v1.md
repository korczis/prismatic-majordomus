---
id: project.rust-public-api-quality
version: 1
kind: rule
title: Every exported Rust item explains itself, shows itself running, and is exercised by something
description: The crate's exported surface is measured from its own syntax tree — documentation that says more than the signature, an executable example on everything that carries behaviour, and a module boundary that something exercises — and what the policy does not ask for is derived and printed rather than exempted by a list.
statement: An item this crate exports is documented in words its signature does not already carry, shown running in an example the toolchain executes, and reached by a test; an item that is none of those is either not finished or not public.
status: active
class: blocking
