...
BEGIN {
    ...
    # `context` is the directory contract — `.ai/**/README.md`, schema context/v1. It was
    # absent here while `sources.yaml`, the Rust index and `majordomus context resolve` all
...
function extract_one(i, k,   p, id, title) {
    ...
    else if (k == "document" || k == "knowledge" || k == "context") title = h1[p]
...
# rule -> each rule it depends on. A dependency is written as `id@version`; the node is the
# rule's identity, so the version is dropped from the reference and kept where it was stated.
function edges_rule(si,   p, id, from, n, v) {
    p = spath[si]
    id = f(p, "id"); if (id == "") return
    from = node_id("rule", id)
    for (n = 0; ; n++) {
        v = f(p, "depends_on." n)
        if (v == "") break
        sub(/@.*$/, "", v)
        add_edge(from, node_id("rule", v), "depends_on", p ":depends_on." n)
    }
}
