...
[`CAPABILITIES.md`](@/docs/capabilities.md) covers the operation model,
[`DYNAMICITY.md`](@/docs/dynamicity.md) covers where a fact lives,
[`DEVELOPMENT_RUNTIME.md`](@/docs/development-runtime.md) covers where a semantic lives,
[`DOCTRINE.md`](@/docs/doctrine.md) covers rules and proof,
[`EVIDENCE.md`](@/docs/evidence.md) covers recorded executions,
[`GITHUB_PAGES_ARCHITECTURE.md`](@/docs/github-pages-architecture.md) covers the website,
[`MESH.md`](@/docs/mesh.md) covers discovery, and [`PLANNING.md`](@/docs/planning.md) covers the plan.
...
- **Worktrees** are identified by their branch. The path `<repo>-wt/<branch>` is derived
  from git and never registered ([`WORKTREES.md`](@/docs/worktrees.md)).
...
([`DEVELOPMENT_RUNTIME.md`](@/docs/development-runtime.md), "The derivation chain"). Unknown
...
generated static mounts such as `/docs/` ([`WEB.md`](@/docs/web.md)). There is no hand-written
...
workflows ([`COMMANDS.md`](@/docs/commands.md)).
...
[`DOCTRINE.md`](@/docs/doctrine.md), the rule files and `.ai/repo/policy.yaml`.
...
| **ADR** | a decision record: `schema: adr/v1`, `status` (`proposed`, `accepted`, `superseded` or `rejected`), reciprocal `supersedes`/`superseded_by`, typed `related` references, `provenance.origin` | one file per decision under `.ai/repo/adrs/`, named by its number and slug | `adr propose` writes `proposed`; acceptance is a person's act, enforced only for records whose origin is `extracted` [C34] |
...
output ([`EVIDENCE.md`](@/docs/evidence.md)).
...
(§16). [`CONTINUITY.md`](@/docs/continuity.md) states the separation: "The episode file records no
