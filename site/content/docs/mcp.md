...
| outdated lease | a lease whose server answers from another version of the code — started from an older build, a release install, another checkout — is taken over, and the log names both versions; a server whose own executable was replaced on disk is `superseded` and taken over the same way, before it is even asked whether it answers |
| self-check | a running server compares its own executable with the file on disk every half second and stops, releasing the lease, when a rebuild has replaced it (`has been replaced on disk` in the log); bridged clients elect again on their next message, so nobody is ever answered by code the tree no longer has |
...
`about.rs` ([`CAPABILITIES.md`](@/docs/capabilities.md)).
...
- the YAML read is the subset [`SCHEMAS.md`](@/docs/schemas.md) defines, so a file the shell tool
...
| `majordomus://scope` | JSON: the repository scope as read, its origin, and every tracked file tallied against it ([`SCOPE.md`](@/docs/scope.md)) |
