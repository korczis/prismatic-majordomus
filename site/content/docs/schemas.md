...
[`DOCTRINE.md`](@/docs/doctrine.md) for the model and `.ai/repo/rules/README.md` for the format
...
tree; today that is `README.md`. See [`CONTEXT.md`](@/docs/context.md) for the model.
...
heading. See [`CLI.md`](@/docs/cli-specification.md) for `majordomus skills`.
...
| `task.gate` | `task`, `gate`, `exit`, `inputs_hash` (over the files the CI model's classes select for that gate) |
| `prompts.reconciled` | `records`, `linked`, `unlinked` |
| `prompts.pruned` | `records`, `pruned`, `bytes_left` |
