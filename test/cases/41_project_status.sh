...
# claims: evidence-gates-done, project-status-derived
. "$ROOT/test/lib.sh"
"$MJ" init >/dev/null
...
grep -q '^started_at: ' .ai/repo/project/issues/I0001.yaml || { echo "    started_at was not recorded"; exit 1; }
expect_no_grep '^status:' .ai/repo/project/issues/I0001.yaml
...
rm .ai/repo/project/issues/I0005.yaml
sed '/^evidence:/,$d' .ai/repo/project/issues/I0001.yaml > /tmp/i.$$ && mv /tmp/i.$$ .ai/repo/project/issues/I0001.yaml
