...
# A path under /tmp is on this side deliberately: it names no account and reads the same on
# every machine, and the repository documents its own fixtures there. What this rule refuses
# is a file naming the machine it was written on, and /tmp names none.
CLEAN=(
  '"worktree": "/tmp/a-fresh-clone" and "worktree": "/tmp/wt"'
