...
out_dist="$LAST_OUT"
# the budget line carries a measured duration, which two runs never share; compare the finding, not the number
a="$(printf '%s\n' "$out_checkout" | grep -E '^(OK|FAIL|WARN)' | sed -e 's/  \[reproduce:.*//' -e 's/ [0-9][0-9]* ms/ N ms/g' | sort)"
b="$(printf '%s\n' "$out_dist" | grep -E '^(OK|FAIL|WARN)' | sed -e 's/  \[reproduce:.*//' -e 's/ [0-9][0-9]* ms/ N ms/g' | sort)"
