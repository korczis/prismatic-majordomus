...
mod tests {
    ...
    fn a_regression_is_never_clamped() {
        ...
        assert!((r + 2.0).abs() < 1e-12, "a treatment three times the control is -200 %, got {r}");
    }
    ...
    fn the_cluster_bootstrap_needs_two_clusters_and_enough_values() {
        ...
        assert!(bootstrap_median_clustered(&many, 9500, 500, 3, 5).is_none(), "one task");
        let few = vec![vec![0.1, 0.2], vec![0.3]];
        assert!(bootstrap_median_clustered(&few, 9500, 500, 3, 5).is_none(), "three values");
        ...
        assert!(bootstrap_median_clustered(&ok, 9500, 0, 3, 5).is_none(), "no resamples");
    ...
    fn the_cluster_bootstrap_is_reproducible_from_its_seed() {
        let c: Vec<Vec<f64>> =
            (0..8).map(|t| (0..4).map(|r| (t as f64 * 0.37 + r as f64 * 0.01).sin()).collect()).collect();
