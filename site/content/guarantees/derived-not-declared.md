...
status = "guaranteed"
source = "docs/claims/derived-not-declared.md"
+++
{% raw %}
...
Fixing the tool removes the trap for everyone, which is why it is the right place to fix it. It does not remove the habit these failures also needed: checking a claim against the tree instead of against a report of the tree, immediately before making it rather than at the start of the work. Several of the instances above were reported, in good faith, from measurements that had been correct an hour earlier. No assertion catches that one.

A sample is not a verdict. `site-probe --quick` measures one route per template, which is sound and deliberate: pages sharing a template share their layout. But it reports "ok" for the routes it visited, and a reader takes that as "the site is fine" — one run passed the sample and the full run then found an overflow on a page the sample had not opened. Nothing was wrong with the sampling; what was missing was the result saying which question it answered. A check that measures part of a thing has to say so in its output, or its "ok" is read as a claim about the whole.

Nor does checking the tree suffice on its own. One of these was found by CI and not reproducible locally, because the generator's input globs expanded in the collation order of whoever ran them: a clean clone of the failing commit reported "in sync" on a developer's machine and STALE on the runner. The commit was identical and the environment was not. Where a result can depend on the environment, the environment that decides is the one to reproduce.
{% endraw %}
