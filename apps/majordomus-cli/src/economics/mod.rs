    ...
    for s in suites.values() {
        for t in &s.tasks {
            if !tasks.contains_key(t) {
                errors.push(format!("suite {} names task {t}, which is not declared", s.id));
            }
        }
    }
...
fn load_context_runs_from(
    ...
    for p in files {
        ...
        match std::fs::read_to_string(&p).map_err(|e| e.to_string()).and_then(|t| {
            ...
            Ok(r) => errors.push(format!("{rel}: schema {} is not {CONTEXT_RUN_SCHEMA}", r.schema)),
            Err(e) => errors.push(format!("{rel}: {e}")),
        }
    }
    runs.sort_by(|a, b| a.measured_at.cmp(&b.measured_at).then(a.repository.commit.cmp(&b.repository.commit)));
    (runs, errors)
}
...
fn judge(
    ...
    let (Some(c), Some(t)) = (control, treatment) else {
        let side = if control.is_none() { "control" } else { "treatment" };
        return (EconomicsPairStatus::Missing, vec![format!("the {side} run is not recorded")]);
    };
    ...
    check("suite version", &c.suite_version.to_string(), &t.suite_version.to_string());
    ...
    check("reported models", &c.models_reported.join(", "), &t.models_reported.join(", "));
    check("fixture digest", &c.fixture_digest, &t.fixture_digest);
    check("harness version", &c.harness.version, &t.harness.version);
    check("configuration digest", &c.configuration_digest, &t.configuration_digest);
    check("repository revision", &c.repository.commit, &t.repository.commit);
    check("session count", &c.sessions.len().to_string(), &t.sessions.len().to_string());
    if !differ.is_empty() {
...
fn pair_status(
    ...
    let ids: Vec<&str> = [control, treatment].into_iter().flatten().map(|r| r.id.as_str()).collect();
...
fn last_session_tokens(r: &EconomicsRun) -> Option<u64> {
    r.sessions.last().and_then(usage::session_totals).map(|t| t.total)
}
...
/// ```
pub fn pairs(decl: &Declarations, suite: &EconomicsSuite, runs: &[EconomicsRun]) -> Vec<EconomicsPair> {
    judged_pairs(decl, suite, runs).into_iter().map(|j| j.pair).collect()
}
    ...
    keys.into_iter()
        .map(|(task, rep)| {
            ...
            let pair = EconomicsPair {
                ...
                task: task.clone(),
                category: declared.map(|d| d.category.clone()).unwrap_or_else(|| "undeclared".into()),
                ...
                first_request_overhead: if computable {
                    let a = c.and_then(|r| r.sessions.first()).and_then(usage::first_request_input);
                    let b = t.and_then(|r| r.sessions.first()).and_then(usage::first_request_input);
...
fn freshness(
    ...
    let now = match inputs_digest(root, &suite.freshness_inputs) {
        Ok(d) => d,
        Err(e) => return (EconomicsFreshness::Stale, Some(format!("the inputs could not be read: {e}"))),
    };
    let stale = recorded
        .iter()
...
fn task_interval(samples: &[Sample], policy: &EconomicsStatisticsPolicy) -> Option<EconomicsInterval> {
    let mut clusters: BTreeMap<&str, Vec<f64>> = BTreeMap::new();
    for s in samples {
        clusters.entry(s.cluster.as_str()).or_default().push(s.value);
    }
...
fn sampled(
    ...
    if m.n > 0 && m.interval.is_none() {
        let tasks = samples.iter().map(|s| s.cluster.as_str()).collect::<BTreeSet<_>>().len();
...
fn change_with_interval(median: f64, interval: Option<&EconomicsInterval>, none: &str) -> String {
    ...
    let range = if percent_text(median).starts_with('-') {
        format!("an increase of {} to {}", bound_text(-i.high), bound_text(-i.low))
    } else {
        format!("a reduction of {} to {}", bound_text(i.low), bound_text(i.high))
    };
...
fn unmet(
    ...
    if valid.len() < policy.min_valid_pairs {
        out.push(format!("{} valid pair(s); the rule asks for {}", valid.len(), policy.min_valid_pairs));
    }
    let mut per_category: BTreeMap<&str, usize> = BTreeMap::new();
    for p in valid {
        *per_category.entry(p.category.as_str()).or_insert(0) += 1;
    }
    let categories = per_category.values().filter(|n| **n >= policy.min_pairs_per_category).count();
    ...
    match interval {
        ...
        Some(i) if (i.high - i.low) * 10_000.0 > policy.max_interval_width_bp as f64 => out.push(format!(
    ...
    if policy.require_current && !all_current {
        out.push("the evidence is not current: a measured mechanism changed since it was recorded".into());
    }
    if dirty > 0 {
        out.push(format!("{dirty} run(s) were recorded from a working tree with uncommitted changes"));
    }
    out
}
...
fn summarize_from(root: &Path, query: &EconomicsQuery, source: Source) -> EconomicsSummary {
        ...
        match suite.kind.as_str() {
            "live" => {
                ...
                let recorded: Vec<(u32, String)> =
                    runs.iter().map(|r| (r.methodology, r.inputs_digest.clone())).collect();
                let (fresh, detail) = freshness(root, &decl, suite, &recorded);
                if !matches!(fresh, EconomicsFreshness::Current | EconomicsFreshness::NoEvidence) {
                    all_current = false;
                }
                let judged = judged_pairs(&decl, suite, &runs);
                let mut counts = EconomicsPairCounts { declared: suite.tasks.len() * suite.repetitions.unwrap_or(1) as usize, ..Default::default() };
                ...
                for p in judged.iter().map(|j| &j.pair).filter(|p| p.status == EconomicsPairStatus::Valid) {
                    if let Some(r) = runs.iter().find(|r| Some(&r.id) == p.control.as_ref()) {
                        if let Some(x) = p.token_reduction {
                            by_rev.entry(r.repository.commit.clone()).or_default().push(x);
                        }
                    }
                }
                for (rev, v) in by_rev {
                ...
                suite_views.push(EconomicsSuiteView {
                    ...
                    runs: runs.len(),
                    pairs: counts,
                    revisions: runs.iter().map(|r| r.repository.commit.clone()).collect::<BTreeSet<_>>().into_iter().collect(),
                    harnesses: runs.iter().map(|r| format!("{} {}", r.harness.name, r.harness.version)).collect::<BTreeSet<_>>().into_iter().collect(),
                    models_reported: runs.iter().flat_map(|r| r.models_reported.clone()).collect::<BTreeSet<_>>().into_iter().collect(),
                });
            ...
            "context" => {
                ...
                let recorded: Vec<(u32, String)> =
                    runs.iter().map(|r| (r.methodology, r.inputs_digest.clone())).collect();
                let latest_only: Vec<(u32, String)> = recorded.last().cloned().into_iter().collect();
                ...
                context_runs.push(ContextEvidence { suite, runs, freshness: fresh });
            }
    ...
    let token_samples = |ps: &[&EconomicsPair]| -> Vec<Sample> {
        ps.iter()
            .filter_map(|p| {
                p.token_reduction.map(|x| Sample { key: pair_key(p), cluster: cluster_of(p), value: x })
            })
            .collect()
    };
    ...
    let models: Vec<String> = valid_runs
        ...
        .into_iter()
        .collect();
    let models_text = if models.is_empty() { "a model the harness did not name".to_string() } else { models.join(", ") };
    let revisions: BTreeSet<&str> = valid_runs.iter().map(|r| r.repository.commit.as_str()).collect();
    let tasks: BTreeSet<(&str, &str)> = valid_all.iter().map(|p| (p.suite.as_str(), p.task.as_str())).collect();
    ...
    if !excluded_runs.is_empty() {
        ...
        let with_excluded: Vec<&EconomicsPair> = all_pairs
            ...
            .map(|j| &j.pair)
            .collect();
        let v: Vec<f64> = token_samples(&with_excluded).into_iter().map(|s| s.value).collect();
    ...
    let narrowed =
        query.suite.is_some() || query.category.is_some() || query.task.is_some() || query.model.is_some();
    let pairs_q: Vec<&Judged> = all_pairs.iter().filter(|j| matches(query, &j.pair, &j.model)).collect();
    ...
    let values = |f: &dyn Fn(&EconomicsPair) -> Option<f64>| -> Vec<Sample> {
        valid
            .iter()
            .filter_map(|p| f(p).map(|x| Sample { key: pair_key(p), cluster: cluster_of(p), value: x }))
            .collect()
    };
    let live_suite = decl
        .suites
    ...
    if primary.n > 0 && !publishable {
        primary.warnings.push("preliminary: the evidence is below the publication rule".into());
    }
    if narrowed {
    ...
    if !m.outliers.excluded.is_empty() {
        let put_back: Vec<&EconomicsPair> = pairs_q
            ...
            .map(|j| &j.pair)
            .collect();
        let excluded_pairs = put_back.iter().filter(|p| p.status == EconomicsPairStatus::Excluded).count();
    ...
    let comparable: Vec<&EconomicsPair> = pairs_q
        ...
        .filter(|p| {
            !matches!(
                p.status,
                EconomicsPairStatus::Missing | EconomicsPairStatus::Incomparable | EconomicsPairStatus::Excluded
            )
        })
        .collect();
    for variant in &m.variants {
        ...
        let per_task: Vec<Sample> = in_valid
            .iter()
            .filter_map(|(p, r)| {
                usage::totals(r).map(|u| Sample { key: r.id.clone(), cluster: cluster_of(p), value: u.total as f64 })
            })
            .collect();
        metrics.push(secondary(sampled(
            metric(
        ...
        let resume: Vec<Sample> = in_valid
            ...
            .filter_map(|(p, r)| {
                ...
                let last_input = prev
                    .requests
                    .last()
                    .map(|q| q.input_tokens + q.cache_creation_input_tokens + q.cache_read_input_tokens)?;
                let first = usage::first_request_input(r.sessions.last()?)?;
                Some(Sample { key: r.id.clone(), cluster: cluster_of(p), value: last_input as f64 - first as f64 })
            })
            .collect();
        let mut cf = sampled(
            metric(
        ...
        cf.warnings.push("counterfactual: modelled from observed values, never a measured saving".into());
        metrics.push(cf);
    }
    ...
    for evidence in &context_runs {
        ...
        if compatible {
            ...
            if latest.repository.dirty {
                standing.push("dirty: this record was measured from a working tree with uncommitted changes".into());
            }
        }
        ...
        let refused = (!latest.refused.is_empty()).then(|| {
            format!("{} seed(s) refused by the compiler were not measured", latest.refused.len())
        });
        let per_seed: Vec<Sample> = if compatible {
            latest
                ...
                .iter()
                .filter_map(|s| {
                    reduction(s.candidate_tokens, s.selected_tokens)
                        .map(|r| Sample { key: s.seed.clone(), cluster: s.seed.clone(), value: r })
                })
                .collect()
        } else {
            Vec::new()
        };
        ratio = sampled(ratio, &per_seed, &m.statistics, Resampling::Independent, false);
        if ratio.n > 0 {
            ...
            ratio.warnings.push("not measured: no seed of the latest record yields a ratio".into());
        } else {
            ratio.warnings.push(incompatible.clone());
        }
        ratio.warnings.extend(refused.iter().cloned());
        metrics.push(ratio);
        let sum = |f: &dyn Fn(&EconomicsContextSeed) -> u64| latest.seeds.iter().map(f).sum::<u64>();
        ...
        if compatible && estimated > 0 {
            ...
            model_error.warnings.push("not measured: the latest record estimates no selected tokens".into());
        } else {
        ...
        context_view = Some(EconomicsContextView {
            ...
            seeds_over_budget_counted: latest.seeds.iter().filter(|s| s.selected_tokens > latest.budget_tokens).count(),
    ...
    for (dimension, key) in [
        ("category", &(|p: &EconomicsPair| p.category.clone()) as &dyn Fn(&EconomicsPair) -> String),
        ("sessions", &|p: &EconomicsPair| format!("{} session(s)", p.sessions)),
        (
            "complexity",
            &|p: &EconomicsPair| decl.tasks.get(&p.task).map(|t| t.complexity.clone()).unwrap_or_default(),
        ),
    ] {
    ...
    history.sort_by(|a, b| (a.suite.as_str(), a.at.as_str(), a.revision.as_str()).cmp(&(b.suite.as_str(), b.at.as_str(), b.revision.as_str())));
...
pub fn explain(root: &Path, metric: &str) -> Result<EconomicsExplanation, String> {
    ...
    let Some(found) = summary.metrics.iter().find(|x| x.id == metric).cloned() else {
        ...
        return Err(format!("no metric {metric:?}; the metrics are: {}", known.join(", ")));
    };
    ...
    let pairs: Vec<EconomicsPair> = if found.class == EconomicsClass::Derived && found.inputs == Some(EconomicsClass::Counted) {
        Vec::new()
        ...
        summary
            ...
            .filter(|p| [&p.control, &p.treatment].into_iter().flatten().any(|id| evidence.contains(id.as_str())))
            .cloned()
            .collect()
    } else {
        summary
            .pairs
            .iter()
            .filter(|p| evidence.contains(pair_key(p).as_str()) || (p.status != EconomicsPairStatus::Valid && of_suite(&p.suite)))
            .cloned()
            .collect()
    };
    ...
    Ok(EconomicsExplanation {
        ...
        suites,
        pairs,
        excluded: decl.map(|d| d.methodology.outliers.excluded).unwrap_or_default(),
...
pub fn runs(root: &Path, q: &EconomicsRunsQuery) -> EconomicsRunList {
    let decl = match load(root) {
        Ok(Some(d)) => d,
        Ok(None) => {
            return EconomicsRunList { count: 0, runs: Vec::new(), diagnostics: Vec::new() }
        }
        Err(e) => return EconomicsRunList { count: 0, runs: Vec::new(), diagnostics: e },
    };
    let mut out = Vec::new();
    let mut diagnostics = Vec::new();
    for suite in decl.suites.values().filter(|s| s.kind == "live") {
        ...
        diagnostics.extend(errors);
        for (path, r) in runs {
            if q.task.as_ref().is_some_and(|t| t != &r.task) || q.variant.as_ref().is_some_and(|v| v != &r.variant) {
                continue;
            }
    ...
    EconomicsRunList { count: out.len(), runs: out, diagnostics }
}
