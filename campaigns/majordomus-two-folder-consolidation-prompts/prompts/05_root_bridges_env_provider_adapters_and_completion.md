...
# Phase 5 — Minimal root bridges, direnv/environment entry, provider adapters and dynamic completion

## Objective

Reduce external/root integration to the smallest stable compatibility surface and ensure every such surface derives from the canonical repository model.

## A. AGENTS.md bridge

Inspect current AGENTS/CLAUDE/provider instruction behavior. Implement the minimum mechanism required for agents to discover canonical `.ai/**` context.

Desired property:

- if `AGENTS.md` already exists, preserve user content and add/update one tiny managed block or equivalent supported include/reference;
- if absent and required for generic agent discovery, create the smallest useful compatibility shim;
- the bridge contains no duplicated rules/doctrines/skills lists;
- the bridge points to the canonical entry point/context loading mechanism;
- ownership is recorded and uninstall-safe.

Do not assume all providers honor the same include syntax. Use adapters where necessary.

## B. `.envrc` / environment bridge

Use the architecture from `references/Dynamic-CLI-banner.txt` as a design constraint, but verify the current repository implementation first.

`.envrc` must remain tiny glue. Preferred shape is a stable bridge to Majordomus env functionality or a generated `.majordomus/env/direnv` adapter. It must not embed repository business logic, git parsing, provider enumeration, curl, cargo builds, documentation generation, or 47 lines of shell archaeology.

Environment entry must:

- use the shared typed repository/environment snapshot,
- avoid network and builds,
- be bounded/cached,
- expose only non-secret provider status,
- support compact/full/auto/off banner behavior if that already exists or is in scope,
- avoid rewriting structural tracked files during directory entry,
- degrade gracefully if the Majordomus binary/service is unavailable.

Treat `.envrc` mutation as externally owned/managed and reversible.

## C. Provider adapters

Normalize provider integration so:

```text
.ai/** canonical semantics
       ↓
RepositoryModel / typed context
       ↓
ProviderAdapter
       ↓
CLAUDE/Codex/Gemini/OpenAI-compatible/local compatibility representation
```

Provider-specific files must live under `.majordomus/generated/providers/**` where possible. If a provider absolutely requires a root/path-specific file, generate the smallest adapter and record ownership.

Adapters must be discoverable/registered via existing generic plugin/provider conventions, not hard-coded into four parallel match statements. Do not expose API keys. Test the distinction between configured/available/unavailable without printing secret values.

## D. Shell completion and `just`

If Majordomus already maps/delegates `just` recipes or exposes dynamic CLI completion, consolidate it now:

- introspect `just` definitions through supported machine-readable output rather than parsing justfile text if practical;
- derive completion candidates and help metadata from the same command/capability registry used by CLI/docs;
- avoid manually listing recipes in `.envrc`, docs, banner, or completion scripts;
- generate/register runtime completion through one stable environment bridge;
- preserve shell portability supported by the project.

No repeated context-unaware completion fragments across shells if they can be generated from one model.

## E. Root hygiene diagnostic

Add/extend `doctor`/validation capability to report unexpected Majordomus-owned root artifacts and legacy compatibility files. It should explain whether an artifact is required, managed, legacy, or user-owned and propose the canonical migration path.

## Tests/performance

Add tests for:

- AGENTS pre-existing content preservation and managed block idempotence,
- `.envrc` pre-existing content preservation and stable bridge,
- provider root adapter generation only when necessary,
- provider secret redaction,
- completion derivation after adding/removing a command/recipe/artifact,
- environment/banner warm and cold budget,
- no network/build during env entry,
- graceful missing-binary/server behavior,
- uninstall of clean bridges and conflict behavior for edited bridges.

## Completion criterion

A target repo's root is boring again. `.ai/**` and `.majordomus/**` carry the architecture; everything outside them is a thin, owned, replaceable shim.
