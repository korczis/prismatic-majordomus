...
contains none. The numbers are in `generated/economics.md`, written
...
- **Total-token change** (`effective_token_reduction`, derived from observed usage). Matched
  live runs of a task corpus, control against treatment. This is the only kind of number
  that could support a claim about what Majordomus saves.
...
1. `majordomus economics run --suite pilot` runs every task in both arms and records one file
   ...
   success gate. Nothing a session said or read is recorded.
2. The calculator pairs control and treatment runs of the same task and repetition, checks
   that they are comparable (suite version, model, fixture, harness version, revision, session
   count) and valid (both succeeded, both report usage), and names the reason for every pair
   that is not.
3. Per valid pair: `reduction = 1 - treatment / control`. A negative value is Majordomus
   costing more, and is reported as it is.
4. Across pairs: the distribution (median, quartiles, 95th percentile, mean, standard
   ...
   same by task category, session count and complexity.
5. The verdict: a quantitative total-token claim is allowed only when every threshold of the
   ...
   enough pairs each, enough repetitions, a high enough valid-pair rate, a narrow enough
   interval, current evidence, and no run from a dirty tree. Below that, every surface states
   that no verified total-token-savings claim is available, beside the preliminary
   observation labelled as preliminary.

No run is excluded unless the methodology names it with a reason, and every aggregate is then
shown with and without it.
