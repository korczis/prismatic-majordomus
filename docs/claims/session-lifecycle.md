# A provider hook opens and closes the execution episode, and the context it was given at the open is frozen beside it
...
Every direction is idempotent, because the events are. `SessionStart` fires again on a resume, so the start passes `--if-open keep` and the already-open episode is kept rather than replaced. `SessionEnd` fires whether or not anything was opened, so the close passes `--if-none ignore` and writes nothing when there is nothing to close. `PreCompact` and `SessionEnd` record what they record whether or not a task is open and whatever outcome the last one reached, because a checkpoint and a continuation record are the episode's and a task is an optional relation they name when they have one. Each event writes a `provider.event.received` line to the ledger before any of that is decided, and a `provider.event.failed` line with a reason when the work does not complete: a hook must return success to its provider, and returning success is not the same as claiming the work happened. Until ADR 0041 all three asked for an `active` task first, and a repository whose last task had been handed over stopped writing both records — for six days, in this one, while every health check passed. The event's reason decides the outcome: a reason the adapter lists as deliberate closes the episode as `closed`, and anything else — a crash, a name the table has not seen — closes it as `interrupted`, because calling a cut-short episode complete is the worse of the two mistakes.

The start hook writes a briefing to standard output and the other two write none. Claude Code adds a `SessionStart` hook's output to the context it is about to build, which is the one moment at which a continuation record reaches a worker without the worker remembering to ask — and a record nothing loads is a record nobody reads. What the briefing may carry is bounded by `session.briefing_budget_lines` and limited to references, divergence labels, the freshness the record is judged at against `session.freshness`, the blockers that refuse acceptance, and the one section of a handover a resuming worker acts on — that last only while the record is still current, because a record past the stale threshold is shown as history with its age and is deliberately not quoted as an instruction; it never carries a conversation, and `session.briefing_on_start: false` restores silence. The end and compaction events fire inside a turn already under way, where output would alter what somebody is doing rather than furnish it, so they say everything on stderr. No hook exits 2, for the same reason `capture prompt` never does.

An episode leaves a continuation record before the envelope closes, and a final checkpoint before that. The two documents answer different questions — the session record indexes what the episode produced, a handover is what the next worker resumes from — and until the end event wrote one, the next worker inherited the first and not the second. Its body is derived by `handover --derive` from the task record, the ledger, git and the open questions, all of which are already written and already validated; nothing calls a model.
