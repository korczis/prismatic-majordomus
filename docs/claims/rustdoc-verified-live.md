...
constant's value, which `build.rs` read from git when the pages were built.
...
`/build.json` built from that commit, the landing page and the library's index answering with their
identities, and the constant page naming that commit. It is its own function beside `verify`,
