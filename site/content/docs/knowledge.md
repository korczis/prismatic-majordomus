+++
title = "Knowledge"
description = "what a knowledge record is, how candidates are derived at every episode boundary from the ledger and git without a model, the promotion act, every command with its reads and writes, the rules that enforce it, and what the next worker is told"
weight = 17
[extra]
...
[`CONTINUITY.md`](@/docs/continuity.md) for the episode and the records that survive it ·
[`CLI.md`](@/docs/cli-specification.md) for every command · [`SCHEMAS.md`](@/docs/schemas.md) for the record format and
the events · [`DOCTRINE.md`](@/docs/doctrine.md) for how the three rules are dispatched and proved ·
[`CAPABILITIES.md`](@/docs/capabilities.md) for how the Rust reader reaches every surface ·
