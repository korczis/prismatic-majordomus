...
head_over_http() { curl -fsS --connect-timeout 5 --max-time 30 "$url/api/v1/repository" | jq -r '.repository.git.head'; }
objects_over_http() { curl -fsS --connect-timeout 5 --max-time 30 "$url/api/v1/repository" | jq -r '.objects'; }
...
mcp() {   # mcp ID METHOD PARAMS [SESSION-HEADER-ARGS...]
  ...
  curl -fsS --connect-timeout 5 --max-time 30 -X POST "$url/mcp" \
