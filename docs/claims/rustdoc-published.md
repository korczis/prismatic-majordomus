# The crate's rustdoc is published at /rustdoc with the site, built from the commit the site names
...
The rustdoc job of the pages workflow produces the tree through the composite action
`.github/actions/rustdoc`, in parallel with the site's own build and check, from the commit being
deployed. The deploy obtains it, `scripts/site-build` composes it at `/rustdoc` from the committed
...
curl -fsS https://majordomus.dev/rustdoc/ | grep -o 'commit <code>[0-9a-f]*'
