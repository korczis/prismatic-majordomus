...
The HTTP projection's own routes, not capabilities: `/`, `/api/v1`, `/cockpit`, `/docs`, `/mcp`, `/openapi.json`. `/docs` is a Swagger UI shell that loads `/openapi.json`; it embeds no specification. `/mcp` is MCP over HTTP on the shared server.
