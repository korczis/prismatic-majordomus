---
id: project.navigation-is-declared-once
version: 1
kind: rule
title: The site's navigation is declared once, and every projection derives from that declaration
description: site/data/nav.toml is the one statement of what the site's navigation is — its groups, their items and the single call to action. Every surface that shows a route a visitor navigates to renders that file; no href is written twice in it, the primary navigation stays small enough to be a set of questions rather than a sitemap, and no template types a site route beside the declaration.
statement: Declare the site's navigation once in site/data/nav.toml and render it everywhere; a route typed into a template is a second navigation that editing the declaration cannot reach, and the same route under two labels is the reader doing the information architecture's job.
status: active
class: blocking
depends_on: [project.derived-once@1, project.interfaces-are-projections@1]
tags: [web, navigation, projections, documentation]

x-majordomus:
  tests: [scripts/ci/nav-check, test/cases/273_navigation_is_declared_once.sh]
---
