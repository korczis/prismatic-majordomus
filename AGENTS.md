<!-- generated by `majordomus update` from .ai/repo/policy.yaml (policy bab445bd2a4c, content f94510ad3ef44592) — do not edit; edit the policy and regenerate -->
# AGENTS.md
...
Done is not a claim. A task is finished when the completion report says so — `majordomus
check` prints where it stands, `majordomus evidence --run-gates` runs and records every
gate the change selects, and `majordomus finish --outcome completed` is refused until every
question of the completion policy (`share/completion.yaml`, the one definition of done)
passes or is exempt: the code, its tests, the documents, the generated artifacts, the
gates, the measured version impact, the trunk, the published surfaces and the live
verification of what they serve. Never report work as done, complete or implemented on
your own account: run `majordomus check`, and say what it said. The stages, each with the
questions that must pass, are:

- Implementation: implemented
- Tests and validation: tested, regression-tested
- Documentation and generated artifacts: documented, generated, openapi, parity
- Commit and push: committed, pushed
- Gates: ci
- Version impact: version, changelog
- Integration: integrated
- Deployment: published, deployed
- Live verification: deployment-verified
- Provenance: handover, issue
