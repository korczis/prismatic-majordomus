...
| **completion policy** | the one definition of done: the lifecycle stages in order and every question a task must answer, each naming the source its answer is taken from — an obligation token, the gate aggregate or one gate, the change set, the structural release analysis, the task record's issue, the continuity store. Shipped with the tool; every surface that states what done means is a projection of it | `share/completion.yaml`, read by `gates.policy` and `gates.completion` |
| **stage** | where a task stands in that lifecycle, folded from the answers: the first stage with a refusing question, else the first with an unanswered one, else complete. Derived at read time and written down by nobody; `complete` — every question passing or exempt — is the one word for done | `.stage` of the completion report, printed by `check`, recorded on `task.finished` |
