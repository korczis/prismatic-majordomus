...
curl -fsS --max-time 30 "$U/api/v1/session/machine" > "$S/m.json" \
...
# ---------------------------------------------------------------- the identities
curl -fsS --max-time 30 "$U/api/v1/session/identity" > "$S/i.json" \
...
[ "$(curl -s --max-time 30 -o /dev/null -w '%{http_code}' "$U/api/v1/session/machine")" = 200 ] \
  || { echo "    the declared route for session_domain.machine is not served"; exit 1; }
[ "$(curl -s --max-time 30 -o /dev/null -w '%{http_code}' "$U/api/v1/session/identity")" = 200 ] \
