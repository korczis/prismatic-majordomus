...
`project.operation-transport-parity` and the working reference is [`QUALITY.md`](QUALITY.md).
...
| `execution` | what running it as an execution means: the effect and whether two may overlap, classified from the kind; and whether asking it to stop achieves anything, which only its handler can say and which `.cancellable()` on the declaration declares ([`EXECUTIONS.md`](EXECUTIONS.md)) |
...
| Swagger UI | a shell page that loads `/openapi.json`; it embeds no specification; its assets are the pinned `swagger-ui-dist` vendored in `share/swagger/`, so the page needs no network | `GET /swagger` (`/docs` is the documentation) |
