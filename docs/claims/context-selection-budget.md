# The assembled context obeys a line budget, and every section it drops is named
...
Lines, not tokens. The briefing is budgeted in lines, which any machine can count deterministically. What the separate context compiler selects is counted in tokens by `majordomus economics measure` (claim `context-selection-counted`); see [`ECONOMICS.md`](../ECONOMICS.md) for what that measures and what it does not.
