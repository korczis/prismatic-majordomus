...
The companion documents are [`DYNAMICITY.md`](DYNAMICITY.md), which decides where a *fact*
lives, and [`CAPABILITIES.md`](CAPABILITIES.md), which decides how an *operation* is
...
| checkpoint | `lib/checkpoint.sh` | `.ai/local/state/checkpoints/` | — | not indexed | `continuity.state`, `context` | `lib/` only | `task.checkpoint` | ” | [`CONTINUITY.md`](CONTINUITY.md) | no capability |
...
| decision (local) | `lib/decision.sh` | `.ai/local/state/decisions.md` | — | not indexed | `context`, `adr propose` | `lib/` only | `decision.recorded` | `test/cases/99_adr.sh` | [`DOCTRINE.md`](DOCTRINE.md) | two names for one concept: `lib/decision.sh` vs `lib/adr.sh`, and `release/changelog.rs` still references `.ai/repo/decisions/` |
