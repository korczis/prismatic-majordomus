...
| **preflight** | what is proven about a checkout, as opposed to what it is: a verdict per claim — git, episode, briefing, task, handover, policy, rules, ADRs, the shared server and its surfaces, peers, recorded test runs, rule enforcement, projections, generated docs, the deployment — each with the evidence it rests on; the value every entry surface renders | `majordomus env preflight`, `/api/v1/environment/preflight`, `majordomus://environment/preflight`, the Cockpit overview |
| **verdict** | `verified`, `active`, `fresh`, `stale`, `degraded`, `unavailable`, `failed`, `unknown`, `not_applicable`: what the repository can show about one claim; the first three cannot be produced without evidence | `verdict:` of a preflight check |
