...
# Phase 4 — `init`/`sync` reconciler, desired state, ownership ledger and safe uninstall

## Objective

Transform `majordomus init` from a template copier or collection of ad hoc mutations into an **idempotent desired-state reconciler** with explicit ownership and safe removal semantics.

## Desired vs actual state

Using the repository model/discovery layer from the prior phase, implement or consolidate typed concepts equivalent to:

```text
ActualRepositoryState
DesiredMajordomusIntegration
Plan / Diff
ApplyResult
ManagedMutation / OwnershipRecord
```

Reuse existing naming/types if present.

The desired integration should normally require only:

- canonical `.ai/**` bootstrap content that truly belongs to the target project,
- minimal `.majordomus/**` manifest/ownership/migration infrastructure,
- tiny root compatibility bridges only when needed.

Do not copy the entire Majordomus repository's own `.ai` corpus into arbitrary target repositories. Seed only generic/project-appropriate bootstrapping content using the existing init semantics.
...
- `majordomus init`
- dry-run/plan mode if the CLI architecture supports it,
- machine-readable plan/result output if consistent with CLI conventions,
- repeated init with zero changes on conforming state,
- detection of partially initialized/legacy state,
- deterministic diagnostics when a human-edited external bridge conflicts with expected managed state.

Do not make init silently destructive.

## Sync semantics

`majordomus sync` or the repository's equivalent should:

1. discover actual state,
2. validate canonical inputs,
3. compute desired managed state,
4. show/produce a deterministic diff,
5. apply only safe/reconciled changes,
6. regenerate disposable materializations,
7. prune only artifacts proven to be managed+obsolete,
8. revalidate,
9. update ownership/migration metadata atomically.

Clearly separate structural reconciliation from ephemeral cache/runtime refresh.

## Ownership ledger

For every mutation outside `.ai/**` and `.majordomus/**`, record sufficient typed data to decide later whether Majordomus can alter/remove it safely. Include the minimal useful set such as:

- path,
- mutation kind (`managed_block`, whole managed file, symlink, etc.),
- stable marker/id,
- original/pre-install fingerprint when applicable,
- last Majordomus-produced fingerprint,
- generator/schema version,
- ownership/lifecycle,
- canonical source/derivation identity,
- migration metadata where needed.

Do not store secrets or entire user files unnecessarily.

### Managed blocks

Prefer delimited managed blocks inside existing user files over replacing the whole file when interoperability allows it. Updating/removing a block must preserve surrounding user content byte-for-byte where practical.

If the managed block itself has been edited by the user, detect conflict and refuse unsafe overwrite/removal with a useful repair/force workflow consistent with project UX.

## Uninstall semantics

Implement a first-class conservative uninstall/deinit path. Default behavior should:

- remove safely owned external root/provider/env bridges,
- stop/remove disposable runtime/generated Majordomus-specific state as appropriate,
- remove `.majordomus/**` when safe or according to existing lifecycle policy,
- **preserve user-authored `.ai/**` semantic knowledge by default**,
- never broad-delete files not proven to be managed,
- warn/refuse on modified managed bridges instead of erasing user edits.

Optional explicit purge modes may remove generated `.ai` seed content or all `.ai` content only with deliberate user intent and clear dry-run output. Follow current CLI safety conventions.

## Atomicity and interruption safety

Plan writes so an interrupted init/sync does not leave a misleading ownership ledger. Use temporary files/atomic rename or existing transactional mechanisms where applicable. Ensure the ownership ledger cannot claim a mutation succeeded before it did.

## Migration versioning

Tie structural layout changes to a versioned migration mechanism. Running current Majordomus against an old repository should produce a deterministic plan, not scatter conditionals throughout unrelated code.

## Tests

Add serious integration/fixture tests for:

- fresh init minimal diff,
- second init zero diff,
- dry-run produces no filesystem change,
- partially initialized legacy repo,
- pre-existing AGENTS/.envrc user content,
- managed block update preserving surrounding content,
- user-edited managed block conflict,
- sync pruning only owned obsolete generated artifacts,
- uninstall default preserves authored `.ai/**`,
- uninstall removes clean managed bridges,
- uninstall refuses unsafe modified bridge removal,
- interrupted/failed apply leaves recoverable state,
- migration from representative legacy layouts,
- machine-readable plan stability if public.

## Completion criterion

`init`, `sync`, and uninstall/deinit now behave like a conservative package manager/reconciler for repository integration, not a pile of `write_file()` calls.
