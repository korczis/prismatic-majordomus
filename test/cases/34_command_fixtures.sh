...
for fx in "$FIX"/*.json; do
  ...
  for sid in $ids; do
    ...
    if [ -n "$stdin" ]; then
      ...
      # no stdin declared means no stdin: a command that reads it gets EOF here, as it would
      # in CI, instead of waiting on a terminal nobody is at
      ( cd "$W" && . "$FIXTURE_SETUP/$setup.sh" >/dev/null 2>&1 && "$MJ" "$@" < /dev/null ) > "$out" 2>&1 || rc=$?
    fi
...
rc=0
# shellcheck disable=SC1090
( cd "$W" && . "$FIXTURE_SETUP/$setup.sh" >/dev/null 2>&1 && "$MJ" "$@" < /dev/null ) >/dev/null 2>&1 || rc=$?
