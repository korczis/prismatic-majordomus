...
status = "guaranteed"
source = "docs/claims/rustdoc-composed-from-the-topology.md"
+++
{% raw %}
...
```bash
jq -r '.surfaces[] | select(.kind == "static-directory")
       | [.id, .mount, .availability] | @tsv' docs/generated/web.json
scripts/site-build               # refuses, naming the producer, until the reference exists
scripts/rust-check --doc         # the producer
scripts/site-build               # "== compose rustdoc at /rustdoc", then the route count
