...
fn a_storm_of_clients_converges_on_one_server_and_one_board() {
    ...
    // and reads the layer through it, and sees the whole board. The board one client
    // reads is a list of peers, each an id with what it announced; naming the shape is
    // what keeps the assertion below readable and what `clippy::type_complexity` asks for.
    type Peer = (String, String, String);
    type Board = (String, Vec<Peer>);
