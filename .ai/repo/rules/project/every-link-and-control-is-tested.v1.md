---
id: project.every-link-and-control-is-tested
version: 1
kind: rule
title: Every link the site publishes resolves, and every control on it does what a spec says
description: No link on the published site is followed to a 404, and no button, field, toggle, tab, filter or scroll region is shipped without a behaviour spec that claims it and a browser run that proves it.
statement: Every href and src the built site emits is decided offline against the tree being published, and every interactive control on every page is claimed by exactly one behaviour spec and exercised by it in a browser; a link or a control added tomorrow is decided and exercised tomorrow, because both sets are discovered from the build rather than listed.
status: active
class: blocking
depends_on: [project.ui-conformance@1, project.derived-once@1, project.no-counts-in-prose@1]
tags: [site, ui, links, accessibility, testing]

x-majordomus:
  tests: [scripts/ci/link-check, scripts/ci/interaction-check, scripts/interaction-probe, test/cases/335_every_link_resolves.sh, test/cases/336_every_control_is_claimed.sh, test/cases/337_every_control_does_what_it_says.sh]
---
