...
/// A rule is judged by the artifacts that can carry proof, not by the weakest path it
/// happens to mention.
///
/// Found by running this against the real corpus: six rules reported `unrunnable` while
/// each named a perfectly good case, because each also named a helper script that no runner
/// and no gate drives as its own command. A false finding is the failure this subsystem
/// exists to refuse, so such a path is reported and judged only when it is all the rule has.
/// A path that is *absent* still dominates: that one is a lie, whatever else is named.
#[test]
fn a_path_nothing_drives_does_not_drag_down_a_rule_that_also_names_a_case() {
    let repo = crate::synthetic::SyntheticRepository::small().unwrap();
    std::fs::create_dir_all(repo.root().join("test/cases")).unwrap();
    std::fs::write(repo.root().join("test/cases/01_alive.sh"), "#!/bin/sh\n").unwrap();
    std::fs::create_dir_all(repo.root().join("scripts")).unwrap();
    std::fs::write(repo.root().join("scripts/helper.sh"), "#!/bin/sh\n").unwrap();

    let write = |stem: &str, block: &str| {
        let rel = format!(".ai/repo/rules/project/{stem}.v1.md");
        let text = std::fs::read_to_string(repo.root().join(&rel)).unwrap();
        std::fs::write(
            repo.root().join(&rel),
            text.replace(
    ...
    assert_eq!(
        both.state,
        RuleState::NotRun,
        "a helper script nothing drives must not make a rule with a real case unprovable"
    );
