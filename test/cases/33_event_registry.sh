...
sed -i.bak '$d' .ai/local/state/ledger.jsonl && rm -f .ai/local/state/ledger.jsonl.bak
expect_exit 0 "$MJ" history --validate
...
expect_grep '"event":"question.opened".*"question":"real"' .ai/local/state/ledger.jsonl
