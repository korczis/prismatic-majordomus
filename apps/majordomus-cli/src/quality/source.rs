...
pub struct Inventory {
    ...
    /// credit back to the module that declares the item. `pub use x as y` offers `y`; a glob,
    /// `pub use m::*`, offers every item of `m` in the re-exporting module and is recorded
    /// as `<module>::*` to `m`.
    ///
    /// The same map is where rustdoc documents a re-exported item whose own module is
    /// private: under the alias, which is why the offered name is the one `as` chose.
...
impl Inventory {
    ...
    pub(crate) fn of_target(crate_dir: &Path, root: &str, krate: &str) -> Result<Self, Error> {
        ...
        walker.file(&file, krate, true, true)?;
...
impl Walker<'_> {
    ...
    ///
    /// `declared_pub` is the visibility its parent's `mod` declaration gives it — the crate
    /// root is `pub` by definition — because a file does not carry its own: the same fact an
    /// inline `pub mod x { .. }` records, so the two ways of writing a module read alike.
    fn file(
        &mut self,
        path: &Path,
        mod_path: &str,
                ...
                syn::Item::Mod(m) => {
                    ...
                    match &m.content {
                        ...
                        None => {
                            ...
                            self.file(&child, &child_path, child_exported, is_pub(&m.vis))?;
                        }
                    }
                }
...
/// is how `pub use docs::tree;` reads. A glob contributes the module itself, offered as
/// `*`, and [`apply_reexports`] takes everything under it.
fn resolve_use(
    tree: &syn::UseTree,
    krate: &str,
    at: &str,
    ...
    match tree {
        ...
        syn::UseTree::Rename(r) => out.push((base(&r.ident.to_string()), r.rename.to_string())),
        // `pub use m::*` offers every item of `m` under its own name in this module, and
        // the module itself under none: the offered name is `*`, which a reader of the
        // aliases replaces with each item's own remainder
