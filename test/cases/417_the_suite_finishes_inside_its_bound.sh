...
#
# So: the two declarations must agree, and scripts/ci/suite-budget must refuse a run that
# costs more than the model allows — and must refuse to call an unmeasurable run a pass.
. "$ROOT/test/lib.sh"
...
echo "    bound ${bound_wf}m declared once; budget ${budget_total}s total, ${budget_slowest}s per case, refused in both directions"
