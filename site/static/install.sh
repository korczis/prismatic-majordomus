...
resolve_release() {
  ...
  if ! http_get "$MJ_METADATA_URL" "$MJ_TMP/release.json"; then
    # A 404 is the project's own answer and means the release is not published. Any other
    # status is the network or the host between here and it, and the two deserve different
    # sentences: one is a fact about the project, the other is a fact about this connection.
    # With wget, which reports no status, neither can be claimed and the older wording stands.
    if [ "$MJ_PINNED" = 1 ]; then
      if [ "${MJ_HTTP_STATUS:-}" = 404 ]; then
        fail "no release ${MJ_WANTED_VERSION} is published" \
             "There is no release metadata at $MJ_METADATA_URL (HTTP 404)." \
             "Every published release is listed at $MJ_BASE_URL/docs/install/."
      fi
      fail "the metadata for ${MJ_WANTED_VERSION} could not be retrieved" \
           "$MJ_METADATA_URL answered${MJ_HTTP_STATUS:+ HTTP $MJ_HTTP_STATUS}, not the release metadata." \
           "This is a failure to reach the release metadata, not a statement that the release" \
           "does not exist. Try again, or check $MJ_BASE_URL/docs/install/."
    fi
    if [ "${MJ_HTTP_STATUS:-}" = 404 ]; then
      fail "no stable release is currently published" \
           "The installer itself is available, but there is no stable release for it to install yet." \
           "Release metadata: $MJ_METADATA_URL (HTTP 404)" \
           "Published releases, when there are any: $MJ_BASE_URL/docs/install/"
    fi
    fail "the release metadata could not be retrieved" \
         "$MJ_METADATA_URL answered${MJ_HTTP_STATUS:+ HTTP $MJ_HTTP_STATUS}, not the release metadata." \
         "This says nothing about whether a release exists; it says this machine could not" \
         "read the metadata that would name it."
  fi
...
# sha256sum where the system has it (Linux, and macOS ships one), openssl otherwise, whose
# "SHA256(file)= <hash>" and "SHA2-256(file)= <hash>" both end in the digest. Never shasum:
# on macOS it is a Perl script, and this installer needs nothing but sh.
sha256_of() {
  file=$1
...
# Fails closed: without a digest tool, or with a digest that is not one, nothing is installed.
# The check is never skipped.
verify_checksum() {
  file=$1 expected=$2
  have sha256sum || have openssl || fail "no way to compute a SHA-256 digest" \
    "This installer needs sha256sum or openssl. It will not install anything it cannot verify."
  actual=$(sha256_of "$file") || actual=""
  case $actual in *[!0-9a-f]*) actual="" ;; esac
  [ "${#actual}" -eq 64 ] || fail "the digest of the download could not be computed" \
    "artifact: $MJ_ARTIFACT" \
    "The digest tool printed no SHA-256. Nothing was installed."
