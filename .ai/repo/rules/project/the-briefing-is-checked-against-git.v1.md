---
id: project.the-briefing-is-checked-against-git
version: 1
kind: rule
title: The briefing an episode opened with is compared against git before it is believed
description: The working context a provider's start hook freezes is a recorded head like every other, so the surfaces that hand it to a worker label it against the current checkout in the vocabulary the rest of the repository already uses, and none of them refuses an episode for having drifted.
statement: Every surface that hands a worker the working context of an open episode states how far that document has drifted from the checkout it describes, using exact, advanced, diverged, different_context and unknown; drift is reported and never refused, and the document itself is never rewritten to make the report come out fresh.
status: active
class: blocking
