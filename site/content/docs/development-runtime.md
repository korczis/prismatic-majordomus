...
|---|---|---|---|---|---|---|---|---|---|---|
| issue | read `src/plan.rs`; **write `lib/plan.sh`** | `.ai/repo/project/issues/*.yaml` | `majordomus.issue/v1` | `sources.yaml` class `issue` | `plan.*` (8 queries), 203 resources, site, GitHub projection | `lib/plan.sh:514-515` only | `plan_start`, `plan_verify`, `plan_evidence`, `plan_done` | `test/cases/*`, `tests/` | [`PLANNING.md`](@/docs/planning.md) | **write half unreachable from every machine surface** |
| milestone | as issue | `.ai/repo/project/milestones/*.yaml` | `majordomus.milestone/v1` | class `milestone` | `plan.model`, `plan.roadmap`, 18 resources | `lib/plan.sh` | as issue | as issue | [`ROADMAP.md`](@/docs/roadmap.md) | as issue |
...
| ADR | `lib/adr.sh` | `.ai/repo/adrs/????-*.md` | `adr/v1` | class `adr` | 38 resources, `graph.get?id=adrs`, `release.changelog` | `lib/adr.sh` | `adr.proposed` | `test/cases/99_adr.sh` | [`DOCTRINE.md`](@/docs/doctrine.md) | no capability; number allocation is unguarded — `0039` is claimed twice across branches today |
...
| execution | `src/execution/` | **in-process only** | typed input per capability | registry | `executions.*`, `/events`, Cockpit | `executions.start`, `.cancel` | 15 stream messages | `tests/` | [`EXECUTIONS.md`](@/docs/executions.md) | **nothing durable; the 15 messages are in no ledger vocabulary** |
| event (durable) | `share/events.yaml` + `mj_ledger_append` | `.ai/local/state/ledger.jsonl` | 20 registered names | registry walk | `history`, `continuity.state`, `obligations` | `lib/` only | itself | `test/cases/*` | [`SCHEMAS.md`](@/docs/schemas.md) | **disjoint from the execution stream in both directions** |
...
| worktree | `src/worktree/` (14 files) | derived from git; lock under `<git-common-dir>/majordomus/locks/` | typed topology | `git worktree list --porcelain` | `worktree.*`, the pre-commit guard, Cockpit | `worktree create/migrate/repair` | — | `tests/`, `test/cases/*` | [`WORKTREES.md`](@/docs/worktrees.md) | two legacy layouts still recognised, deliberately |
...
| repository / index | `src/repository.rs`, `src/index.rs` | `.ai/manifest.yaml` + the trees it names | `manifest/v1` | manifest, then `sources.yaml` | everything | — | — | `tests/` | [`SCHEMAS.md`](@/docs/schemas.md) | 4 on-disk sections (`benchmarks`, `ci`, `providers`, `workspaces`) are in no manifest section, plus 6 loose baseline files |
...
|---|---|---|---|---|---|---|
| rule / doctrine | `.ai/repo/rules/{project,vendor}/` | tracked Markdown, identity `id`+`version` | `check`, `finish`, `doctor`, `watch`, site, 120 resources | `lib/rules.sh` (vendor update) | [`DOCTRINE.md`](@/docs/doctrine.md) | 120 effective rules — 51 vendored, 69 project; 92 blocking, 28 advisory; 40 tool-enforced and **0 of the 69 project rules is tool-enforced**, which is the doctrine, not a defect: a project rule gets a CI gate |
...
| use case | `.ai/repo/use-cases/*.md` | tracked | 43 resources, coverage gate, site | `lib/usecase.sh` | [`USE_CASES.md`](@/docs/use-cases.md) | `lib/usecase.sh` is a **second writer of `docs/generated/`** |
...
| quality / gates | `.ai/repo/ci/gates.yaml` | tracked | `validate.yml`, `just gate` | hand-edited + `ci-plan --check` | [`CI.md`](@/docs/ci.md) | 36 gates, 3 on demand; 152 shell cases and 40 Rust test files |
