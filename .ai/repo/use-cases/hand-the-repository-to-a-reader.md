...
summary: 'Take a snapshot that a model can read or a gate can run in, without sending anything untracked and without losing the file modes on the way.'
category: knowledge
status: active
target: guaranteed
weight: 60
...
commands: [archive]
doctrines: []
claims: [archive-fidelity, no-network]
