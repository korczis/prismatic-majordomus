...
///
/// Not "read-only": the registry holds commands whose declared effect is a repository
/// mutation, and a sentence every client reads first must not deny them. Which ones they
/// are is measured by the projection that serves them ([`writes`]), never written here.
pub const SUMMARY: &str = "The projection of a repository's AI layer: every rule, prompt, session and project record under .ai/, the capabilities that read them and the declared commands that change them, served over MCP, HTTP and the command line from one registry.";
...
pub const PARAGRAPHS: &[&str] = &[
    ...
    "**Loopback, unauthenticated, and explicit about what writes.** The first `majordomus mcp` in a repository binds this server beside its stdio session and logs the URL; `majordomus serve` binds it alone. It listens on 127.0.0.1 and asks for no credentials: whoever can reach the socket can read the layer and can run every command it exposes. An operation that changes a tracked file declares the effect `repository_mutation` in `x-majordomus-effect`; it is always a `POST` and never a read-only MCP tool, and every other operation reads. `GET /` is the home page and lists every surface; `/swagger` is Swagger UI over this document; `/docs/` is this repository's own documentation; `/mcp` is MCP over HTTP for a second client.",
...
/// The sentence that says what writes the repository, given the MCP tools whose declared
/// effect is a repository mutation, as the projection measured them from its registry.
///
/// ```
/// use majordomus_cli::about;
/// assert_eq!(about::writes(&[]), "Nothing here writes to the repository.");
/// let text = about::writes(&["majordomus_a", "majordomus_b"]);
/// assert!(text.starts_with("Only majordomus_a, majordomus_b write to the repository"));
/// ```
pub fn writes(tools: &[&str]) -> String {
    if tools.is_empty() {
        return "Nothing here writes to the repository.".to_string();
    }
    format!(
        "Only {} write to the repository (declared effect repository_mutation); every other tool and resource reads.",
        tools.join(", ")
    )
}
