...
| the crate's reference | the rustdoc pages of `apps/majordomus-cli`, composed at `/rustdoc` ([`RUSTDOC.md`](@/docs/rustdoc.md)) | `target/web/rustdoc/`, with the `surface.json` that declares what it was built from | never — `scripts/rust-check --doc`, not committed |
...
reference is absent, broken or stale fails the run. [`RUSTDOC.md`](@/docs/rustdoc.md) has the whole
...
pages the build just produced, the composed surfaces' among them (the crate's reference at
`/rustdoc`), so it lists no route of its own. Each page has its route, its
