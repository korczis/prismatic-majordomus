---
id: project.ui-conformance
version: 1
kind: rule
title: Every page of every web surface is audited, and the audit names no page
description: The user interface is held to one executable standard — responsive at every width the theme declares, keyboard-operable, and WCAG 2.1 AA — over a page set and a width set that are discovered rather than listed.
statement: Every web-accessible page is audited at every width the compiled theme implies, against invariants an engine checks rather than a reviewer remembers; the page set, the width set and the remediation of generated markup are derived, so a page added tomorrow is audited tomorrow without a list changing.
status: active
class: blocking
...
arithmetic. ADR 0017 records the decision and what it rejected.
